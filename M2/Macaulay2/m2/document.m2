...
verifyKey Sequence := key -> ( -- e.g., (res, Module) or (symbol **, Module, Module)
    ...
    and not any({Keyword, Command, Function, ScriptedFunction, Functor}, type -> instance(key#0, type)) and not methodNames#?(key#0)
    and not (instance(key#0, Sequence) and 2 == #key#0 and key#0#1 === symbol= and instance(key#0#0, Keyword))
    then error("documentation key ", format toString key, " encountered, but ", format toString key#0, " is not a function, command, scripted function, functor, or keyword");
    --
