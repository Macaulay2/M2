...
assert ( num == rank source g )				    -- failing
...
assert ( num == rank source h )				    -- failing
leadTerm mingens M
leadTerm relations trim M
leadTerm matrix { elements (set columns h - set columns mingens M) }
leadTerm matrix { elements (set columns mingens M - set columns h) }
