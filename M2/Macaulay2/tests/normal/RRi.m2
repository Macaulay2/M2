...
I = interval(1,3)
assert not identical(I,interval(3,1))
assert isEmpty interval(3,1)
assert identical(I,span(3,1))
assert identical(I,interval[1,3])
...
assert(identical(I+J,J+I))
assert(identical(I*J,J*I))
K = interval(-2,3)
assert(not identical(K*K,K^2))
...
assert identical(abs(K),interval(0,3))
assert (midpoint(I) == J)
assert (diameter(I) == 2)
assert isMember(numeric_100 pi,numeric_100 piRRi)
L = interval(-2/3,1.5_(RR_53))
M = interval(-4,2.7)
assert identical(L+M,interval(-14/3,4.2))
...
assert identical(L/I,L)
assert identical(L^2,interval(0,2.25))
...
assert isMember(numeric_100 pi,acos cos(numeric_100 piRRi))
