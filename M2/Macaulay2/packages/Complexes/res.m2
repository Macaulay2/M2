...
resolutionBySchreyerFrame := (opts, M) -> (
	...
	M.cache.resolution = complex m); -- FIXME: chainComplex ==> complex here.
    i := length C;
    m = C.dd_i;
...
resolutionBySyzygies := (opts, M) -> (
    ...
    else M.cache.resolution = complex presentation M; -- FIXME: this usage of complexes is no longer available.
    i := length C;
    m := C.dd_i;
    while i < opts.LengthLimit and m != 0 do (
...
isComputationDone ResolutionComputation := Boolean => options resolution >> opts -> container -> (
    ...
    C := if instance(container.Result, Complex) then container.Result else return false;
...
updateComputation(ResolutionComputation, Complex) := Complex => options resolution >> opts -> (container, result) -> (
...
resolution Ideal := Complex => opts -> I -> resolution(
