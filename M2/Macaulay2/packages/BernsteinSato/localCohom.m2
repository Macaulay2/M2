...
localCohom(      LeftIdeal) := HashTable => o ->    I  -> localCohom(toList (0..numgens I), I, o)
localCohom(ZZ,   LeftIdeal) := HashTable => o -> (n,I) -> (localCohom({n}, I, o))#n
localCohom(List, LeftIdeal) := HashTable => o -> (l,I) -> (
     ...
     then localCohomUli (l,I)
	  ...
	  R := ring I;
	  createDpairs R;
	  localCohom(l, I, coker gens ideal R.dpairVars#1, o)
	  )
     );
...
localCohomUli = (l, I) -> (
     ...
     C#0 = directSum { {} => coker gens ideal W.dpairVars#1 };
     apply(toList(1..r), k->(
	       C#k = directSum (select(subISets, u -> #u == k) / (theta -> (
			      theta => coker gens J#theta 
			      )));
...
localCohom(      LeftIdeal, Module) := HashTable => o ->   (I,M) -> localCohom(toList (0..numgens I), I, M, o)
localCohom(ZZ,   LeftIdeal, Module) := HashTable => o -> (n,I,M) -> localCohom({n}, I, M, o)
localCohom(List, LeftIdeal, Module) := HashTable => o -> (l,I,M) -> (
     -- Promote I to the Weyl algebra if it is not already there
     if #(ring I).monoid.Options.WeylAlgebra == 0
     then (
...
localCohomILOaku(List, LeftIdeal, Module) := (l, I, M) -> (
     -- error checking to be added
     -- I is assumed to be an ideal in WA generated by polynomials
     f := first entries gens I;
...
localCohomILOTW(List, LeftIdeal, Module) := (l, I, M) -> (
     -- error checking to be added
     -- I is assumed to be an ideal in WA generated by polynomials
     f := first entries gens I;
     FT := theta -> product(theta, i->f#i);
...
localCohomRegular(List,LeftIdeal,Module) := (l, I, M) -> (
     -- error checking to be added
     -- I is assumed to be an ideal in WA generated by polynomials
     f := first entries gens I;
     FT := theta -> product(theta, i->f#i);
...
localCohomOT = method()
localCohomOT(LeftIdeal, LeftIdeal) := (I, J) -> (
     if not J.?quotient then J.quotient = coker gens J;
     localCohomOT(I, J.quotient)
     )
localCohomOT(LeftIdeal, Module) := (I, M) -> computeLocalCohomOT(I, M, 0, numgens I)

localCohomOT(List, LeftIdeal, Module) := (l, I, M) -> (
...
time h = localCohom I
time h = localCohom (I, coker gens J, Strategy=>Walther)
...
L' = localCohom (m, coker gens K, Strategy=>OaTa);
assert (L'#0==coker gens ideal(x^3,y^3));

---------------------------------------------------------------
x = symbol x; dx = symbol dx; 
W = QQ[x, dx, WeylAlgebra=>{x=>dx}]
I = ideal {x, x^2, x^2+x, x^3, x^4+2*x}
M = coker gens ideal dx 
