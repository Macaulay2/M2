...
localCohom(List, Ideal) := HashTable => o -> (l,I) -> (
     W := makeWA ring I; -- TODO: what degrees are best for the differentials?
     if (o.Strategy == Walther and o.LocStrategy === null)
     then localCohomUli(l, sub(I, W))
     else (
	  createDpairs W;
	  localCohom(l, I, comodule ideal W.dpairVars#1, o)
	  )
     );
...
localCohomUli = (l, I) -> (
	 ...
	 C := applyValues(subISets, S -> directSum apply(S, theta -> (theta => comodule J#theta)));
...
localCohomOT = method()
localCohomOT(Ideal, LeftIdeal) := (I, J) -> (
     if not J.?quotient then J.quotient = comodule J;
     localCohomOT(sub(I, ring J), J.quotient)
     )
...
time h = localCohom I
time h = localCohom (I, comodule J, Strategy=>Walther)
...
assert (sub((minimalPrimes charIdeal Mat)_0,W)==sub(m, W));

K = ideal(x^3,y^3);
L' = localCohom (m, comodule K, Strategy=>OaTa);
assert (L'#0 == comodule sub(K, W));
