...
newPackage(
    ...
    DebuggingMode => false,
    PackageImports => {"Elimination", "LLLBases"}
)
-------------------------------------------------------------------------------
...
export{
    ...
    "rationalPoints", "Amount", "Split", "Bound", "KeepAll"
}
-------------------------------------------------------------------------------
...
ht := x -> (
    ...
    -- Compute an LLL basis to get the norm of the gcd (which is an integral ideal instead of an integer!)
    M := LLL getIntegralMatrix x;
    m := numcols M;
    finitePlaces := 1 / abs det M_{m-d..m-1};
    place := (p, x) -> max(x / (xi -> abs embed_p xi)); -- this is less accurate than `baseChange` but a lot faster
    infinitePlaces := times toSequence for p in primitiveElementValues k list place_p x;
...
round2Integers := F -> (
    ...
    for fct in factor abs D do if (last fct // 2 > 0) then (
        ...
        U := (map(F, modp, {a})) (T'//Z);
        M := hermite (lift(getMatrix apply(result_{0..m-1}, w->w*U), ZZ) | p*id_(ZZ^n));
        result = first entries (1/p*matrix{result}*sub(M_{m..m+n-1}, F));
        if (last fct // 2) >= m+1 then while true do (
            ...
            m = #kerC;
            if m == 0 then break;
            M = hermite (lift(getMatrix kerC, ZZ) | p*id_(ZZ^n));
            result = first entries (1/p*matrix{result}*sub(M_{m..m+n-1}, F));
        );
    );
...
-------------------------------------------------------------------------------
-- Get the matrix of an integral ideal, not in normal form!
-- To compute the norm, it suffices to compute an LLL basis and take the
-- determinant; one can also obtain the Hermite normal form but it is slower.
--
getOneIntegralMatrix := x -> (
    F := ring x;
    n := extensionDegree F;
    a := primitiveElement F;
    x = (toBaseRing F) x;
    return lift(sum apply(n, i->(multiTable F)_i*coefficient(a^i, x)), ZZ);
);
getOneIntegralMatrix = memoize getOneIntegralMatrix;
getIntegralMatrix = x -> (
    x = select(x, xi->xi != 0);
    Ms := apply(x, xi->getOneIntegralMatrix xi);
    M := first Ms;
    for i in (1..<#Ms) do M = M | Ms_i;
    -- M := transpose@@matrix@@flatten \\ entries \ transpose \ Ms;
    return M;
);
-------------------------------------------------------------------------------
...
hermiteNormalForm List := x -> (
    ...
    x = apply(x, xi->xi*denom);
    -- Result of `hermite` not always positive!
    -- But it's fine when #columns > #rows, hence this hack...
    if #x == 1 then x = x|x;
    M := hermite getIntegralMatrix x;
    m := numcols M;
    return (1/denom)*sub(M_{m-n..m-1}, QQ);
);
-------------------------------------------------------------------------------
...
bddHeight = (k, B) -> (
    ...
    F << "from sage.all import NumberField, PolynomialRing, QQ, RR\n"
      ...
      << "K = NumberField("|replace("\\^", "**", toExternalString p)|", 'a')\n"
...
getZeros := (coeffs) -> (
    ...
    try ( -- try use `factor` first
        ...
        k' := baseRing k;
        R' := k'(monoid[gens R]);
        p = sum apply(#coeffs, i -> (toBaseRing k) coeffs_i * R'_0^i);
...
Description
 Text
  This method computes the Hermite normal form of a fractional ideal using {\tt
  hermite} from the package @TO "LLLBases"@.
 Example
