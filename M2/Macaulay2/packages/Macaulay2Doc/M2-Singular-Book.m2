document { 
	Key => "M2SingularBook",
    	Headline => "Macaulay 2 examples for the Singular book",
    	EM "M2SingularBook", " consists of Macaulay 2 translations of the examples in the book ",
	      EM "A Singular introduction to commutative algebra",
	PARA{},
	"Each example is numbered as it is in Greuel and Pfister's book.  Links to 
	Macaulay 2 routines and concepts are also provided. ",
	PARA{},
	-- Mike wanted this: TO "Macaulay 2 implementation bugs and problems",
	Subnodes => {
	     "Chapter 1",
...
-- document {
--      Key => "Macaulay 2 implementation bugs and problems",
--      UL {
...
document {
     ...
     to find the dimension, Macaulay 2 requires the Groebner basis of I.  It computes this 
...
document {
     ...
     "Normal forms in Macaulay 2 are done using the remainder operator ", TO symbol%, ".",
     EXAMPLE {
	  "R = QQ[x,y,z];",
     ...
     "Macaulay 2 computes a Groebner basis of G, and uses that to find the
     normal form of f. 
...
document {
     ...
     "Intersecting ideals using the Macaulay 2 ", TO intersect, " function.",
     EXAMPLE {
	  "A = QQ[x,y,z];",
	  "I1 = ideal(x,y);",
	  "I2 = ideal(y^2,z);",
...
"M = matrix{{1, x+y, z^2},
     ...
     "Indices in Macaulay 2 are always 0 based, so the upper left
...
document {
     ...
     "Notice that the basis that Macaulay 2 uses for Hom(A^3,A^2) is different than the basis
     used by Singular.",
     PARA{},
...
document {
     ...
     "A common method of creating a submodule of A^n in Macaulay 2 is to take the image
...
document {
     ...
     "Macaulay 2 has a ", TT "modulo", " command (it was initially introduced in the original Macaulay, 
     in the late 1980's), but it is not needed very often.  It is used internally in Macaulay 2 to implement
