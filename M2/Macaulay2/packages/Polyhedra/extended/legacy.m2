...
smallestFace(Matrix,Cone) := (p,C) -> (
     ...
     if contains(C,coneFromRays p) then (
	  M := halfspaces C;
     	  N := hyperplanes C;
...
tailCone Polyhedron := P -> coneFromRays(rays(P),linSpace(P))
...
normalCone (Polyhedron,Polyhedron) := Cone => opts -> (P,Q) -> (
     ...
     if not P.cache.normalCone#?Q then (
	  ...
	  if not isFace(Q,P) then error("The second polyhedron must be a face of the first one");
	  p := interiorPoint Q;
	  P.cache.normalCone#Q = dualCone coneFromRays affineImage(P,-p));
