...
TEST ///
-- plueckerIdeal and plueckerMap on the diagonal matching field of Gr(2,4).
-- The Plucker ideal of Gr(2,4) is the principal ideal generated by the
-- classical Plucker relation p_(1,2) p_(3,4) - p_(1,3) p_(2,4) + p_(1,4) p_(2,3),
-- and that relation lies in the kernel of the Plucker map.
MF = diagonalMatchingField(2, 4);
P = plueckerIdeal MF;
assert(instance(P, Ideal));
assert(numgens P == 1);
phi = plueckerMap MF;
assert(instance(phi, RingMap));
assert(source phi === ring P);
assert((phi P)_0 == 0);
///

TEST ///
-- matchingFieldPolytope and NOBody.  The matching-field polytope of the
-- diagonal Grassmannian Gr(k,n) has C(n,k) vertices (one per Plucker
-- variable), and for the diagonal matching field the Newton-Okounkov body
-- has the same dimension as the matching-field polytope.
MF24 = diagonalMatchingField(2, 4);
Q24 = matchingFieldPolytope MF24;
assert(instance(Q24, Polyhedron));
assert(numColumns vertices Q24 == 6);
MF35 = diagonalMatchingField(3, 5);
Q35 = matchingFieldPolytope MF35;
assert(numColumns vertices Q35 == 10);
N24 = NOBody MF24;
assert(instance(N24, Polyhedron));
assert(dim N24 == dim Q24);
///

TEST ///
-- flMatchingField (Matrix constructor) and matchingFieldRingMap.
M = matrix{{0,0,0,0},{1,2,3,4},{1,4,9,16}};
flMF = flMatchingField M;
assert(instance(flMF, FlMatchingField));
mfm = matchingFieldRingMap diagonalMatchingField(2, 4);
assert(instance(mfm, RingMap));
///

TEST ///
-- linearSpanTropCone and matroidSubdivision return the expected structures.
MF = diagonalMatchingField(2, 4);
assert(instance(linearSpanTropCone MF, Module));
MS = matroidSubdivision MF;
assert(instance(MS, List));
assert(#MS > 0);
///
