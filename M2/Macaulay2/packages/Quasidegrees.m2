...
doc ///
  ...
  Description
   ...
   Text
    Start with a small multigraded module and compute its quasidegree set directly.
   Example
    A = matrix{{1,0},{0,1}}
...
-- Cover makeGradedRing, toGradedRing, toricIdeal, quasidegreesAsVariables,
-- quasidegrees, and the qav/qlc aliases directly. These exports were
-- previously documented but only exercised indirectly inside the two
-- holonomicRank tests above (both of which depend on HolonomicSystems).
TEST ///
A := matrix{{1, 2, 3}, {1, 2, 2}};
R := makeGradedRing(A, symbol z);
assert(class R === PolynomialRing);
assert(numgens R == 3);
-- The columns of A become the degree vectors of the variables.
assert(degrees R == {{1, 1}, {2, 2}, {3, 2}});
-- toGradedRing recreates the same grading on a different polynomial
-- ring with the same number of variables.
S := QQ[a, b, c];
Sg := toGradedRing(A, S);
assert(degrees Sg == {{1, 1}, {2, 2}, {3, 2}});
-- The toric ideal for this A is generated by the single binomial
-- z_0^2 - z_1.
I := toricIdeal(A, R);
assert(class I === Ideal);
assert(numgens I == 1);
-- quasidegrees and quasidegreesAsVariables on a positively-graded
-- monomial-quotient module.
Rpos := QQ[x, y, Degrees => {{1, 0}, {0, 1}}];
J := ideal(x^2 * y, x * y^2, y^3);
M := Rpos^1 / J;
qd := quasidegrees M;
assert(class qd === List);
assert(all(qd, p -> #p == 2));
qav1 := quasidegreesAsVariables M;
qav2 := qav M;
-- qav is the exported alias for quasidegreesAsVariables.
assert(qav1 == qav2);
assert(class qav1 === List);
-- qlc is the exported alias for quasidegreesLocalCohomology.
qlc1 := quasidegreesLocalCohomology M;
qlc2 := qlc M;
assert(qlc1 == qlc2);
///
