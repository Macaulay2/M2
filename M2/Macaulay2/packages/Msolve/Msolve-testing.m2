needsPackage("Msolve", Configuration => {
        "msolveBinaryFolder" => "~/src/git-from-others/msolve/msolve"}
    )

nextPrime (2^16 + 3)
p = 65539
R = ZZ/p[
    
gens gb I    

restart
needsPackage "Msolve"

TEST ///
  R=ZZ/1073741827[z_1..z_3]
  I=ideal(7*z_1*z_2+5*z_2*z_3+z_3^2+z_1+5*z_3+10,8*z_1^2+13*z_1*z_3+10*z_3^2+z_2+z_1)
  ans = gens gb I
  gB=grobBasis I
  assert(ans == gB)
  lT=monomialIdeal leadTerm I
  assert(lT == monomialIdeal(z_1^2, z_1*z_2, z_1*z_3^2, z_2^2*z_3^2))
  assert(degree lT == 4)
  assert(dim lT == 1)
  ///

TEST ///
  R=QQ[z_1..z_3]
  I=ideal(7*z_1*z_2+5*z_2*z_3+z_3^2+z_1+5*z_3+10,8*z_1^2+13*z_1*z_3+10*z_3^2+z_2+z_1)
  ans = gens gb I
  gB=grobBasis I
  assert(ans == gB)
  lT=monomialIdeal leadTerm I
  assert(lT == monomialIdeal(z_1^2, z_1*z_2, z_1*z_3^2, z_2^2*z_3^2))
  assert(degree lT == 4)
  assert(dim lT == 1)
  ///

TEST ///
S = ZZ/1073741827[t12,t13,t14,t23,t24,t34];
I = ideal((t13*t12-t23)*(1-t14)+(t14*t12-t24)*(1-t13) - (t12+1)*(1-t13)*(1-t14), (t23*t12-t13)*(1-t24)+(t24*t12-t14)*(1-t23) - (t12+1)*(1-t23)*(1-t24), (t14*t34-t13)*(1-t24)+(t24*t34-t23)*(1-t14) - (t34+1)*(1-t14)*(1-t24));
sat=(1-t24);
J1= saturate(I,sat);
J2=ideal saturateByPoly(I,sat);
assert(J1==J2);

  R=ZZ/1073741827[z_1..z_3]
  I=ideal(7*z_1*z_2+5*z_2*z_3+z_3^2+z_1+5*z_3+10,8*z_1^2+13*z_1*z_3+10*z_3^2+z_2+z_1)
  ans = gens gb I
  gB=grobBasis I
  assert(ans == gB)
  lT=monomialIdeal leadTerm I
  assert(lT == monomialIdeal(z_1^2, z_1*z_2, z_1*z_3^2, z_2^2*z_3^2))
  assert(degree lT == 4)
  assert(dim lT == 1)
  ///

TEST ///
  
///
    
filename = "/var/folders/wb/8v4mm0j52pq9pf5gkr8f23z40000gr/T/M2-93265-0/1.ms"
debug Core
gB2 = map(R, rawMatrixReadMsolveFile(raw R, filename))
assert(gB2 == gB)

