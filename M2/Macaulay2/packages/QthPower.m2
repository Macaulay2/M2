...
--   INPUT : weight matrix, 
--           multivarite polynomial ring with given Noether normalization,P, 
--	    (with monomial order generated by the weight matrix),
--	     and generating set for the ideal defining
--           a type I affine domain 
--          (or at least a similar type of integral extension) 
--  OUTPUT : weight matrix, 
--           multivariate polynomial ring 
--	    (with grevlew-over-weight monomial ordering)
--	     and a Gr\"obner basis for a strict affine P-algebra
--	     presentation for the integral closure, 
--	    (and fractions describing the map back to the input ring)
--COMMENTS :(1) The package contains short functions 
...
--              though there is more structure 
--              if one tackles subdomains separately.
...
--          (4) The author's view is that it is worth the effort to
--              preprocess the input of an integral closure problem,
--              so that there is more induced structure on the output.
--          (5) While there is a minimization function below,
--              it is currently probably better to use the weights
--              to define a new ring with appropriate names,
...
--------------------------------------------------------------------------------
-- reconstructing fractions from a/b from remainders c mod N, N>a^2+b^2 
...
document {
     ...
     -- UPDATE HISTORY : 11 February 2010, 8 September 2010
     ---------------------------------------------------------------------------
     PARA {
...
document {
     ...
     PARA {
	  ...
	  There is a grevlexWeight method in this package, 
	  but it is not exported since it is not needed explicitly.
	  While it is the implicit monomial ordering on the integral closure
	  outputs, the choice was to output only the weight matrix defining
	  it or the weightGrevlex monomial ordering."
	  }
     }	
----------------------------------------------------------------------------
document {
...
-----------------------------------------------------------------------------------
 ...
 document {
     ...
     EXAMPLE lines ///
          ...
          ///,
     PARA {
     	  "This gives a quick conductor element 
...
-----------------------------------------------------------------------------------
 document {
     ...
     PARA {
	  ...
	  either qthIntegralCcosure or rationalIntegralClosure.
...
time ic1 = qthIntegralClosure(wtR2,Rq,Iq);toString(ic1)
...
Iq = {y^12+y^11+y^10*x^2+y^8*x^9+x^31};
time ic1 = qthIntegralClosure(wtR6,Rq,Iq); toString(ic1)
time ic2 = minimization(ic1); toString(ic2)
time ic3 = qthIntegralClosure(ic2#0,ic2#1,ic2#2); toString(ic3)
...
time ic1 = qthIntegralClosure(wtR9,Rq,Iq); toString(ic1)
time ic2 = minimization(ic1); toString(ic2)
time ic3 = qthIntegralClosure(ic2#0,ic2#1,ic2#2); toString(ic3)
...
time ic1 = qthIntegralClosure(wtR11,Rq,Iq); toString(ic1)
time ic2 = minimization(ic1); toString(ic2)
time ic3 = qthIntegralClosure(ic2#0,ic2#1,ic2#2); toString(ic3)
