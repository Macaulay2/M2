...
{*
Copyright (C) [2012] [David Eisenbud, Janko Boehm, Max Nitsche]
...
export {binomialIdeal, monomialAlgebra,
       decomposeMA,decomposeHomogeneousMA,
       ...
       randomSemigroup,Num,SetSeed,Simplicial}
...
decomposeMA=method(Options=>{Verbose=>0,ReturnMingens=>false,CoefficientField=>ZZ/101})
decomposeMA RingMap := opt-> f -> (
  vb:=opt.Verbose;
  S:=target f;
  P:=source f;
  K:= coefficientRing S;
...
decomposeMA PolynomialRing :=opt-> R ->(
  K:= coefficientRing R;
  B := degrees R;
...
isSeminormalMA PolynomialRing := R ->(
  if not isSimplicialMA R then error("Expected simplicial monomial algebra");
  fBA:=decomposeMA(R,ReturnMingens=>true);
  f:=fBA#0;BA:=fBA#1;
...
TEST ///
      L=randomSemigroup(5,3,7,Num=>2,SetSeed=>true)
...
doc ///
  ...
  Description
    Text
      ...
      @TO decomposeMA@ -- Decomposition of one monomial algebra over a subalgebra.
      ...
      In order to use 4ti2 you have to configure the Macaulay2 package {\it FourTiTwo} first. On most systems it should work
      ...
      The following files contain the tuples (regularity, degree, codim) for every semigroup B in \mathbb{N}^d with fixed {\bf d} and fixed coodinate sum {\bf a}. This gives a computer proof of the Eisenbud-Goto conjecture in these cases.
...
doc ///
  Key
    decomposeMA
    (decomposeMA,RingMap)
    (decomposeMA,PolynomialRing)
    (decomposeMA,List)
    (decomposeMA,List,List)
  Headline
...
doc ///
  ...
  Description
   Text
    ...
    created by @TO decomposeMA@, @TO decomposeHomogeneousMA@.
...
doc ///
  ...
  Inputs
    ...
    B  : List
           ...
           to convert it into a list of elements of \mathbb{N}^2.
    dc : HashTable
           Using this argument the decomposition computed by @TO decomposeMA@ can be specified if known a priori.
  Outputs
    :ZZ
...
doc ///
  ...
  Description
   Text
     ...
     The option @TO SetSeed@ can be used to control the random number generator.
   ...
   Example
     ...
     randomSemigroup(5,3,7,SetSeed=>true)
     randomSemigroup(5,3,7,SetSeed=>true)
     randomSemigroup(5,3,7,SetSeed=>true,Num=>2)
     randomSemigroup(5,3,7,SetSeed=>true,Simplicial=>true)
///
...
doc ///
  ...
  Headline
    Option of randomSemigroup to return a simplicial semigroup.
  Description
   Text
    Option of @TO randomSemigroup@ to return a set of generators of a simplicial semigroup
...
doc ///
  Key
    ...
    [randomSemigroup,Num]
  Headline
    Option of randomSemigroup to return a list of several semigroups.
  Description
   Text
    Option of @TO randomSemigroup@ to return a list of several semigroups.
...
doc ///
  ...
  Headline
    Option to set the random seed for randomSemigroup.
  Description
   Text
    Option to set the random seed for @TO randomSemigroup@.
