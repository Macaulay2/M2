-- undocumented methods and symbols... 
undocumented {
    }
...
doc ///
  ...
  Description
    Text
      ...
      $k[x_1,\ldots,x_n] / I$ generated by $f_1, \ldots, f_k$. We say $f_1, \ldots, f_k$ 
      form a {\it subalgebra basis} with respect to a monomial order $<$ if the 
      {\it initial algebra} associated to $<$, defined as 
      $in(R) := k[in(f) \mid f \in R] \subseteq k[x_1, \ldots, x_n] / in(I)$, 
      is generated by the elements $in(f_1), \ldots , in(f_k)$. The main functions 
      provided by this package are for computing these subalgebra bases: @TO sagbi @, 
      @TO subalgebraBasis@, and @TO isSAGBI @.
    Text
       {\bf Some references for Subalgebra bases (aka canonical subalgebra bases, SAGBI bases)}

      @UL {
      "Kapur, D., Madlener, K. (1989). A completion procedure for computing a canonical basis 
      of a $k$-subalgebra. Proceedings of Computers and Mathematics 89 (eds. Kaltofen and Watt), 
      MIT, Cambridge, June 1989", "Robbiano, L., Sweedler, M. (1990). Subalgebra bases,
      ...
      "Stillman, Michael, and Harrison Tsai. Using SAGBI bases to compute invariants. J. Pure and 
      Appl. Alg., 1999, pp.~285--302.", "B. Sturmfels, Groebner bases and Convex Polytopes, 
      Univ. Lecture Series 8, Amer Math Soc, Providence, 1996"
      }@
  SeeAlso
...
doc ///
   ...
   Inputs
     ...
     AutoSubduce=>Boolean
       a flag indicating when to perform autosubduction on the generators before performing 
       the Sagbi basis computation (See: @TO "AutoSubduce"@)
     ReduceNewGenerators=>Boolean
       a flag indicating whether to apply gaussian elimination to new sagbi generators before 
       adding them to the current sagbi basis (See: @TO "ReduceNewGenerators"@)
     ...
     Strategy=>String
       ...
       The strategy \"Master\" is a hybrid that combines the other two:
     ...
     AutoSubduceOnPartialCompletion=>Boolean
       ...
       Use this only if very few new sagbi generators are expected. 
       (See: @TO "AutoSubduceOnPartialCompletion"@)
     PrintLevel=>ZZ
       If greater than zero, information is printed about the progress of the computation 
       (See: @TO "PrintLevel"@)
     Recompute=>Boolean
       if @ TT "true" @ then the computation will resume otherwise it starts at the beginning
     RenewOptions=>Boolean
       if @ TT "true" @ then the computation will use the options specified otherwise it will 
       use the previously selected options (except for the following, which may always be
       specified: @TO "PrintLevel"@, @TO "Limit"@, @TO "Recompute"@, and @TO "RenewOptions"@)
   Outputs
     N:Matrix
       whose entries form a partial subalgebra basis
   Description
     Text
       The command @TO "subalgebraBasis"@ @TT "A"@ is equivalent to the command 
       @TO gens@ @TO "sagbi"@ @TT "A"@.
   SeeAlso
     sagbi
     AutoSubduce
     ReduceNewGenerators
...
doc ///
   ...
   Inputs
     ...
     AutoSubduce=>Boolean
       ...
       the sagbi basis computation (See: @TO "AutoSubduce"@)
     ReduceNewGenerators=>Boolean
       a flag indicating whether to apply gaussian elimination to new sagbi generators 
       before adding them to the current sagbi basis (See: @TO "ReduceNewGenerators"@)
     StorePending=>Boolean
       a flag that indicates whether the {\it pending list} should be stored in the result. 
       (See: @TO "StorePending"@)
     Strategy=>String
     ...
     AutoSubduceOnPartialCompletion=>Boolean
       ...
       (See: @TO "AutoSubduceOnPartialCompletion"@)
     PrintLevel=>ZZ
       When this is greater than zero, information is printed about the progress of the 
       computation (See: @TO "PrintLevel"@)
     Recompute=>Boolean
       if @ TT "true" @ then the computation will resume otherwise it starts at the beginning
     RenewOptions=>Boolean
       if @ TT "true" @ then the computation will use the options specified otherwise it will use
       the previously selected options (except for the following, which may always be specified: 
   ...
   Description
	...
	isSAGBI gS
    Text
        Partial subalgebra bases are unavoidable since a subalgebra of a polynomial ring, 
	endowed with some polynomial order, need not have a finite subalgebra basis.  
...
doc ///
   ...
   Description
     Text
       An {\it autosubduction} is when a collection of ring elements (usually a set of generators 
       or subalgebra generators) are subducted against each other, one at a time. The result of 
       each subduction replaces the original element that was subducted.
       If any element subducts to zero, then it is removed from the collection.
        
       By default, the function @TO "sagbi"@ performs autosubduction on the generators of a subring
...
doc ///
   ...
   Description
     Text
       ...
       However if ReduceNewGenerators is set to @ TT "true" @ (by default) then the new generators 
       are reduced using gaussian elimination against the other new generators.
       This process is quick (as fast as computing a groebner basis for a linear ideal) and 
       ensures that duplicate generators are not added to the list of sagbi generators.
   SeeAlso
     sagbi
     AutoSubduce
     ReduceNewGenerators
...
doc ///
   ...
   Description
     Text
       ...
       If the sagbi algorithm terminates without producing a complete sagbi basis, for example due to 
       @TO "Limit"@, then the computation may be resumed by calling @TO "sagbi"@ on the resulting computation 
       object with new options. The resumed computation will use the pending list stored in the @TO "SAGBIBasis"@ computation object.
...
doc ///
   ...
   Description
     Text
       ...
       The \"Incremental\" strategy computes a full groebner basis of the {\it reduction ideal} by using the previous 
       full groebner basis together with the data of the new sagbi generators.
       ...
       By setting Strategy to \"Master\" (default), the \"DegreeByDegree\" strategy is used at the beginning 
       and the strategy switches to \"Incremental\" part of the way through the algorithm.
   SeeAlso
     sagbi
     AutoSubduce
     ReduceNewGenerators
...
doc ///
   ...
   Description
     Text
       ...
       To view each subduction taking place inside the Sagbi algorithm, use a high value for @TO PrintLevel@ 
       together with top-level subduction.
   SeeAlso
     sagbi
     AutoSubduce
     ReduceNewGenerators
...
doc ///
   ...
   Description
     Text
       The stopping criteria for the main loop in @TO "sagbi"@ are: a finite sagbi basis is computed, 
       or the smallest degree of an {\it S-pair} exceeds the @TO Limit@.
       In general, the initial terms of generators of a partial computation do not generate the initial algebra 
       up to the degree limit. It is possible that, with a higher degree limit, sagbi generators of lower degree will be added.
       One can guarantee that all sagbi generators up to a certain degree are found by using a graded term-order,
        such as GRevLex and working with @ ofClass Subring @ of @ ofClass PolynomialRing @ (and not @ ofClass QuotientRing @).
   SeeAlso
     sagbi
     AutoSubduce
     ReduceNewGenerators
...
doc ///
   ...
   Description
     Text
       ...
       PrintLevel 1+: some basic information each loop (no polynomials): computation degree, number of S-pairs, 
       number of new generators, termination conditions.
       ...
       Higher levels exist but may be overly verbose. PrintLevel 5 shows the processPending data,
       and PrintLevel 6 shows the intermediate steps for top-level subduction.
   SeeAlso
     sagbi
     AutoSubduce
     ReduceNewGenerators
...
doc ///
   ...
   Inputs
     ...
     AutoSubduce=>Boolean
       a flag indicating when to perform autosubduction on the generators before performing the Sagbi basis computation 
       (See: @TO "AutoSubduce"@)
     ReduceNewGenerators=>Boolean
       a flag indicating whether to apply gaussian elimination to new sagbi generators before adding them to the current 
       sagbi basis (See: @TO "ReduceNewGenerators"@)
     StorePending=>Boolean
     ...
     Strategy=>String
       ...
       The strategy \"Master\" is a hybrid that combines the other two; starting with \"DegreeByDegree\" for low degrees 
       and switching to \"Incremental\". (See: @TO "Strategy"@)
     SubductionMethod=>String
       the method used for subduction either: \"Top\" or \"Engine\". (See: @TO "SubductionMethod"@)
     Limit=>ZZ
       a degree limit for the binomial S-pairs that are computed internally.
...
doc ///
   ...
   Description
     Text
       ...
       After running @ TO "sagbi" @ on @ ofClass Subring @, 
       the result of the computation is stored in the @ TT "cache" @ of that subring as @ ofClass SAGBIBasis @.
       ...
       a test to see if the generators form a sagbi basis. This test can be skipped by setting 
       @TO "Compute"@ to @ TT "false" @, and, in that case, @TT "isSAGBI"@ will return @ TT "null" @.
     Example
       R = QQ[x,y,z];
...
doc ///
   ...
   Inputs
     ...
     Strategy=>String
       the update strategy at the beginning of each loop: \"DegreeByDegree\", \"Incremental\", and \"Master\".
       The strategy \"Master\" is a hybrid that combines the other two; starting with 
       \"DegreeByDegree\" for low degrees and switching to \"Incremental\". (See: @TO "Strategy"@)
     SubductionMethod=>String
       the method used for subduction either: \"Top\" or \"Engine\". (See: @TO "SubductionMethod"@)
     SAGBILimitType=>String
       Either \"Fixed\" or \"Function\". Determines the stopping criterion for the sagbi computation. 
       If \"Fixed\" then the @TO "Limit"@ is used. Otherwise if \"Function\" is selected,
...
doc ///
   ...
   Description
     Text
        @TT "IntersectedSubring"@ is a type of @TO "Subring"@ produced by intersecting two subrings 
	using the function @TO "subringIntersection"@.
...
doc ///
   ...
   Description
     Text
       Returns two single-row matrices of the generators of the original subrings whose intersection, 
       via the function @TO "subringIntersection"@, is $S$.
     Example
       R = QQ[x,y];
       I = ideal(x^3 + x*y^2 + y^3);
       Q = R/I;
...
doc ///
   ...
   Description
     Text
        ...
        @TT "Subring"@ is a type that stores information associated to a subring of a polynomial ring, 
	...
	An instance of a @TT "Subring"@ is constructed with the function @TO "subring"@.
       ...
       @UL {
	    ...
	    {BOLD {"cache"}, ": Contains unspecified information. The contents of the cache may effect performance, 
		                but should never effect the result of a computation."},
...
doc ///
    ...
    Description
	    ...
	    the adjoint action of $SE(3)$, as considered in the preprint @HREF("https://arxiv.org/abs/2001.05417", 
