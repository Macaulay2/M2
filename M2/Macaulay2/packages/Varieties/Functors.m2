...
-- TODO: should this also check that the variety is of finite type over the field?
checkVariety = (X, F) -> (
...
twistedGlobalSectionsModule = (F, bound) -> (
    -- compute global sections module H^0(X, F(>= bound)), for a CoherentSheaf F.
...
-----------------------------------------------------------------------------
-- hh: Coherent sheaf cohomology or Hodge decomposition
-----------------------------------------------------------------------------
...
Complex LowerBound := SumOfTwistsComplex => (C, b) -> (checkProjective variety C; new SumOfTwists from {C, b})
...
--
-- We allow the option hh^i(C, Degree => a) to mean hh^i(C(a)). We expect people to use the latter notation,
-- but (in effect) we translate it into the first notation for better caching. (We only need to store information
-- about one complex, not its twists.)
--
hh(ZZ, SumOfTwistsComplex) := Sequence => opts -> (cohodeg, sumoftwists) -> (
    -- Compute H^{cohodeg}(X,C(a)) for all integers a, as a sum of two Laurent polynomials,
    ...
    --if C.cache.?Twist then return hh^cohodeg((C.cache.Twist#1)(*), Degree => opts.Degree + first C.cache.Twist#0);
    ...
    -- Our formula for the Hilbert series of H^i(P, C~(*)) (where i = cohodeg) is:
    --   hilb(H^i(C)) - phi(hilb(im(Ext^(n-i)(tau^{>=i}C, w) -> Ext^(n-i)(C, w))))
    --   + phi(hilb(Ext^(n-i-1)(C, w))) - phi(hilb(im(Ext^(n-1-i)(tau^{>=i+1}C, w) -> Ext^(n-i-1)(C, w)))).
    -- Here phi: Z[T,T^(-1)] -> Z[T,T^(-1)] is the ring homomorphism with phi(T) = T^(-1),
    -- and C -> tau^(>=j)C is the canonical truncation.
    mapi := inducedMap(canonicalTruncation(C,infinity,-i),C);
    mapiplus1 := inducedMap(canonicalTruncation(C,infinity,-i-1),C);
    Cresmap := resolutionMap C;
    ...
    rightpart := hilbertSeries(HH^i(C), Reduce => true);
    denom := denominator hilbertSeries R1;
    poincimageextmapi := poincare image HH^(n-i)(Hom(mapofresi, w));
    poincimageextmapiplus1 := poincare image HH^(n-1-i)(Hom(mapofresiplus1, w));
    poincextiplus1 := poincare HH^(n-i-1)(Hom(source Cresmap, w));
    leftpart := substitute(reduceHilbert Divide{-poincimageextmapi+poincextiplus1-pointimageextmapiplus1, denom}, T => U);
...
cohomology(ZZ,                    CoherentSheaf) := Module => opts -> (p,    F) -> HH^p(variety F,F)
...
-- See the documentation of Ext^ZZ(CoherentSheaf,CoherentSheaf) for examples.
-----------------------------------------------------------------------------
