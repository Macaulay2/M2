...
-------------------------------------------------------------------
localCohom = method(Options => {Strategy => Walther})
...
----------------------------------------------------------------------------------------
localCohom(Ideal) := o -> I -> (
     localCohom(toList (0..numgens I), I, o)
);

localCohom(ZZ, Ideal) := o -> (l,I) -> (
     localCohom({l}, I, o)
     );
localCohom(List, Ideal) := o -> (l,I) -> (
     if o.Strategy == Walther then localCohomUli (l,I)
     else (
	  R := ring I;
	  createDpairs R;
...
localCohom(Ideal, Module) := o -> (I, M) -> (
     localCohom(toList (0..numgens I), I, M, o)
);

localCohom(ZZ, Ideal, Module) := o -> (l,I,M) -> (
     localCohom({l}, I, M, o)
     );
localCohom(List, Ideal, Module) := o -> (l,I,M) -> (
     if o.Strategy == Walther then localCohomRegular(l,I,M)
     else if o.Strategy == OT then localCohomOT(l,I,M)
     else if o.Strategy == WaltherOTW then localCohomILOTW(l,I,M)
     else if o.Strategy == WaltherOaku then localCohomILOaku(l,I,M)
	       ...
	       scan(indices C#(k-1), i0->(
		    	 scan(indices C#k, j0->(
				   if isSubset(i0, j0) 
					...
					gj := (L#j0).generator;
					if gj#1 > gi#1
					then if o.Strategy == WaltherOTW then error "Bad luck!"
					else error "Bad luck!"; -- have to fix that: go back and recalculate 
					                        -- the localizations
				        TempM = TempM 
					+ (C#k)_[j0] 
					* map(L#j0#LocModule, L#i0#LocModule, 
...
localCohomOT(Ideal, Ideal) := (I, J) -> (
     if (J.cache.?quotient === false) then J.cache.quotient = (ring J)^1/J;
     localCohomOT(I, J.cache.quotient)
     )
