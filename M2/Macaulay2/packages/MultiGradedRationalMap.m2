...
doc ///
    ...
    Headline
        computes the saturated special fiber ring as a quotient ring
    Usage
        satSpecialFiber(I)
        satSpecialFiber(I, nsteps)
    Inputs
    ...
    Outputs
        : Ring
            the saturated special fiber ring (as a quotient ring whose defining ideal is @TO satSpecialFiberIdeal@)
    Description
       Text
         This function returns the saturated special fiber ring as a quotient ring;
         the defining equations are computed by @TO satSpecialFiberIdeal@.
       Example
         R = QQ[x,y,z]
         A = matrix{ {x, x^5 + y^5},
                     {-y, y^5 + z*x^2*y^2},
...
doc ///
  ...
  Caveat
    To call the method "upperBoundDegreeSingleGraded(I)", the ideal $I$ should be in a single graded polynomial ring and dim(R/I) <= 1.
///
...
TEST ///
    ...
    I = ideal(x*u, y*v, x*v + y*u) -- a non birational map
    assert(degreeOfMapIter(I, 5) == 2)
    A = matrix{ {x^5*u,  x^2*v^2},
                {y^5*v, x^2*u^2},
                {0,     y^2*v^2}
              };
    I = minors(2, A)  -- a non birational
    assert(degreeOfMapIter(I, 5) == 10)
...
TEST ///
     ...
     I = minors(2, A) -- a non birational map
     assert(length gensSatSpecialFib I == 6)
     assert(length gensSatSpecialFib(I,5) == 6)
     R = QQ[x,y,u,v, Degrees => {{1,0}, {1,0}, {0,1}, {0,1}}]
     I = ideal(x*u, y*v, x*v + y*u) -- a non birational map
     assert(length gensSatSpecialFib(I,5) == 4)
...
TEST ///
     ...
     I = minors(2, A) -- a non birational map
     assert(numgens satSpecialFiberIdeal I == 10)
     R = QQ[x,y,u,v, Degrees => {{1,0}, {1,0}, {0,1}, {0,1}}]
     I = ideal(x*u, y*u, y*v) -- a birational map
     ...
     I = ideal(x*u, y*v, x*v + y*u) -- a non birational map
     satSpecialFiberIdeal(I, 5)
     assert(numgens satSpecialFiberIdeal(I,5) == 1)
