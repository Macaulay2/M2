...
specialCubicFourfold (EmbeddedProjectiveVariety,EmbeddedProjectiveVariety) := o -> (S,X) -> (
    ...
    if i >= 1 then if not assertSmoothness X then error "expected a smooth cubic fourfold";
    n := o.NumNodes;
    ...
    if i == 2 or (i >= 3 and n > 0) then (
        if not assertSmoothness(S * random(1,0_S)) then error "expected a surface with at most a finite number of nodes";
    ...
    if i >= 3 then (
        if n > 0 then (
            ...
            if not assertSmoothness(Var source q) then error "expected a surface with smooth normalization";
...
parametrize SpecialCubicFourfold := X -> (
    ...
    Psi := fanoMap X;
    X.cache#"rationalParametrization" = inverse3(Psi|ring X)
);
...
specialGushelMukaiFourfold (EmbeddedProjectiveVariety,EmbeddedProjectiveVariety) := o -> (S,X) -> (
    ...
    if i >= 1 then if not assertSmoothness X then error "expected a smooth GM fourfold";
    if i >= 2 then (
        if not assertSmoothness S then error "expected a smooth surface";
        if o.Verbose then <<"-- smoothness of the surface verified (assuming equidimensionality)"<<endl;
    );
    if i >= 4 then (
        if S != top S then error "expected an irreducible reduced surface";
...
parametrizeFanoFourfold EmbeddedProjectiveVariety := o -> X -> (
    ...
    if degree X == 5 and sectionalGenus X == 1 then (
        ...
        return multirationalMap inverse3 rationalMap(sub(ideal C,ring X),1);
    );
...
inverse3 MultirationalMap := Psi -> (
    ...
    multirationalMap inverse3 toRationalMap Psi
);
...
assertSmoothness = method(); -- sufficient conditions for smoothness ('X' is assumed to be equidimensional)
assertSmoothness EmbeddedProjectiveVariety := X -> (
...
numberNodes EmbeddedProjectiveVariety := o -> X -> (
    if X.cache#?"FiniteNumberOfNodes" then return X.cache#"FiniteNumberOfNodes";
    if o.Verbose then <<"-- computing number of nodes using a probabilistic method... "<<endl;
    if coefficientRing X === QQ then X = X ** (ZZ/nextPrime random(1000,11000000));
    if codim linearSpan X > 0 then X = (parametrize linearSpan X)^^ X;
    if dim singLocus X == -1 then return 0;
    if dim singLocus X >= 1 then error "expected at most a finite number of nodes";
    n := dimdegree radical ideal (rationalMap random({{1},{1}},0_X)) singLocus X;
    if o.Verbose then <<"-- got "<< n <<" nodes"<<endl;
    X.cache#"FiniteNumberOfNodes" = n
);
...
toRationalMap = method();
toRationalMap MultirationalMap := Phi -> (
    f := rationalMap toRingMap(Phi,ring target Phi);
    assert(# factor Phi == 1 and instance(f,RationalMap));
    f
);
...
References => {
PARA{EM"Some new rational Gushel fourfolds",", available at ",HREF{"https://arxiv.org/abs/2003.07809","arXiv:2003.07809"}," (2020)."},
PARA{EM"On some families of Gushel-Mukai fourfolds",", available at ",HREF{"https://arxiv.org/abs/2002.07026","arXiv:2002.07026"}," (2020)."},
PARA{EM"New examples of rational Gushel-Mukai fourfolds",", available at ",HREF{"https://arxiv.org/abs/1910.12838","arXiv:1910.12838"}," (2020)."},
PARA{EM"Trisecant Flops, their associated K3 surfaces and the rationality of some Fano fourfolds",", available at ",HREF{"https://arxiv.org/abs/1909.01263","arXiv:1909.01263"}," (2020)."},
PARA{EM"Explicit rationality of some cubic fourfolds",", available at ",HREF{"https://arxiv.org/abs/1811.03502","arXiv:1811.03502"}," (2019)."},
PARA{EM"Congruences of 5-secant conics and the rationality of some admissible cubic fourfolds",", available at ",HREF{"https://arxiv.org/abs/1707.00999","arXiv:1707.00999"}," (2018)."}}}
...
SeeAlso => {(toGrass, EmbeddedProjectiveVariety)}} 
...
EXAMPLE{"-- A general GM fourfold of discriminant 20"|newline|"X = specialGushelMukaiFourfold(\"surface of degree 9 and genus 2\",ZZ/33331);", "describe X", "time f = detectCongruence X;", "Y = grassmannianHull X; -- del Pezzo fivefold containing X", "p = point Y -- random point on Y", "time C = f p -- 3-secant conic to the surface", "S = surface X;", "assert(dim C == 1 and degree C == 2 and dim(C*S) == 0 and degree(C*S) == 3 and isSubset(p,C) and isSubset(C,Y))"}, 
SeeAlso => {coneOfLines}} 
...
document {Key => {(specialCubicFourfold, EmbeddedProjectiveVariety), (specialCubicFourfold, Ideal), (specialCubicFourfold, String, Ring), (specialCubicFourfold, String)}, 
...
Inputs => {"X" => SpecialGushelMukaiFourfold}, 
Outputs => {EmbeddedProjectiveVariety => {"a fivefold of degree 5 ",TEX///$Y\subset\mathbb{P}^8$///," such that ",TEX///$X\subset Y$///," is a quadric section (the fourfold ",TEX///$X$///," is of ordinary type if and only if ",TEX///$Y$///," is smooth)"}}}, 
...
EXAMPLE {"K = ZZ/10000019; S = random({{3},{1},{1}},0_(PP_K^5));", "X = specialCubicFourfold S;", "time f = unirationalParametrization X;", "describe f", "image f == ideal X", "degreeMap f"}, 
