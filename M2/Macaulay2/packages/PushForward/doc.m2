...
Outputs
    M :Module
        $A$-module which is isomorphic to $B^1$ made into an $A$-module via $f$
Description
    Text
        If $f: A \to B$ is a ring map, and $B$ is finitely generated as an $A$-module,
        then the function returns the module $M$ with  $M \cong B^1$ as $A$-modules.
    Example
        kk = QQ;
        S = kk[a..d];
...
Key
    ...
    (pushFwdGens, Module)
Headline
    get matrix of generators for a pushFwd module as elements of the module that was pushed
Usage
    gs = pushFwdGens M
Inputs
    M: Module
        of the form @TT "pushFwd(f, N)"@ for some RingMap $f: A \rightarrow B$ and some $B$-module $M$
Outputs
    gs: Matrix
        $R^m \rightarrow N$, representing the $m$ elements of $N$ corresponding to an $S$-generating set for $M$
Description
    Example
        kk = ZZ/3
        R = kk[a..b]
