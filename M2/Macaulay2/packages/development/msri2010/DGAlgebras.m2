...
export {DGAlgebra, dgAlgebra, setDiff, natural, cycles, DGAlgebraMap,
	homologyClass, getBasis, koszulComplexDGA, acyclicClosure, toComplex,
	killCycles, getGenerators, adjoinVariables, deviations,
...
-- [code v1] Clean up the code, and document the internal functions.
...
setDiff (DGAlgebra,List) := (A,diffList) -> (
   ...
   A.dd = (toComplex(A,totalOddDegree(A)+1)).dd;
   A
)
...
-- this function is broken at this time
getBasis (ZZ,Module) := opts -> (homDegree,M) -> (
   local retVal;
   R := ring M;
   myMap := map(R, R.cache.basisAlgebra);
   gensM := sub(gens M, R.cache.basisAlgebra);
   relsM := sub(relations M, R.cache.basisAlgebra);
   M' := image gensM / image relsM;
   tempList := (flatten entries basis(homDegree, M', Limit => opts.Limit)) / myMap;
   error "err";
   if tempList == {} then retVal = map((R)^1,(R)^0, 0) else
   (
      -- tempList = reverse sort tempList;
      tempList = select(tempList, z -> z != 0);
      degList := apply(tempList, m -> -degree m);
      retVal = map(R^1, R^degList, matrix {tempList});
   );
   retVal
...
getDegNModule = method()
getDegNModule (ZZ,Ring,Module) := (n,R,M) -> (
   ...
   if not A.cache#?basisAlgebra then (
      ...
      A.cache#basisAlgebra = tempRing/substitute(ideal A,tempRing);
   );
   myBasis := getBasis(n,M);
   ...
   myRowDegree := apply(rowDegList_myCols, h -> -drop(h,1));
   tempMatrix = map(R^myRowDegree, R^myColDegree,substitute(tempMatrix,R));
   coker tempMatrix
)

getDegNModule (ZZ,Ring,Ring) := (n,R,A) -> getDegNModule(n,R,A^1)

TEST ///
-- not sure how to get this to work.  Need to talk to Mike about it.
restart
...
R = ZZ/101[x]
A = R[t]
M = ideal{x^4*t^3, x^2*t^4}/ideal{x^5*t^5}
M1 = getDegNModule(3,R,M)
M2 = getDegNModule(4,R,M)
///
...
torAlgebra = method(TypicalValue=>Ring, Options=>{GenDegreeLimit=>-1,RelDegreeLimit=>-1})
torAlgebra Ring := opts -> R -> (
  ...
  n := 3;
  if opts.GenDegreeLimit != -1 then n = opts.GenDegreeLimit;
...
makeHomologyRing (DGAlgebra,List,List) := opts -> (A, cycleList, relList) -> (
  ...
  if relList == {} then (
     ...
     if opts.ForceGB then forceGB gens I else if A.isHomogeneous then I = ideal gens gb I else I = ideal gens gb I;
     HA = polyRing/I;
...
getCycleProductMatrix (DGAlgebra,List,ZZ) := (A,cycleList,N) -> getCycleProductMatrix(A,makeHomologyRing(A,cycleList,{}),cycleList,N)
...
homologyAlgebra = method(TypicalValue=>Ring,Options=>{GenDegreeLimit=>-1,RelDegreeLimit=>-1})
homologyAlgebra DGAlgebra := opts -> A -> (
  local HA;
  maxDeg := maxDegree A;
  
  if maxDeg == infinity and (opts.GenDegreeLimit == -1 or opts.RelDegreeLimit == -1) then
     error "Must supply upper degree bound on generators and relations if there is a DG algebra generator of even degree.";
  if opts.GenDegreeLimit != -1 then maxDeg = opts.GenDegreeLimit;
  
  n := maxDeg;
  while n <= maxDeg and prune homology(n,A) == 0 do n = n - 1;
  maxHomologyDegree := n;
  if opts.RelDegreeLimit != -1 then maxHomologyDegree = opts.RelDegreeLimit;
...
isHomologyAlgebraTrivial = method(TypicalValue=>Boolean,Options=>{GenDegreeLimit=>-1})
...
isGolodHomomorphism = method(TypicalValue=>Boolean,Options=>{GenDegreeLimit=>-1})
...
findTrivialMasseyOperation = method(TypicalValue=>List, Options=>{GenDegreeLimit=>-1,TMOLimit=>2})
findTrivialMasseyOperation DGAlgebra := opts -> A -> (
   maxDeg := maxDegree A;
   if maxDeg == infinity and opts.GenDegreeLimit == -1 then error "Must specify an upper bound on the generating degree";
   if opts.GenDegreeLimit != -1 then maxDeg = opts.GenDegreeLimit; 
   cycleList := getGenerators(A,DegreeLimit=>maxDeg);
...
homologyClass = method(TypicalValue=>RingElement)
homologyClass RingElement := z -> (
   z
)
...
doc ///
  ...
  Description
    ...
    Example
      ...
      M = coker matrix {{a^3*b^3*c^3*d^3}};
      S = R/ideal{a^3*b^3*c^3*d^3}
      A = acyclicClosure(R,EndDegree=>6)
      B = A ** S
...
-- test 0 : not really a test, just keeping the code for producing the Taylor resolution as an algebra somewhere.
restart
loadPackage "DGAlgebras"
R = ZZ/101[a,b]
...
TEST ///
-- test 1 : isHomogeneous, toComplex, maxDegree
R = ZZ/101[x,y,z]
...
TEST ///
--- test 3 : homology, homologyAlgebra, HH_ZZ, HH
...
TEST ///
-- test 6 : homology of a DGA whose H_0 is not a field
R = ZZ/32003[a,b]
I = ideal{a^6,b^6}
A = koszulComplexDGA(I)
HA = HH A
...
TEST ///
-- test 8 : acyclicClosure, isHomologyAlgebraTrivial, isGolod, isGolodHomomorphism
R = ZZ/101[a,b,c,d]/ideal{a^4,b^4,c^4,d^4}
M = coker matrix {{a^3*b^3*c^3*d^3}};
S = R/ideal{a^3*b^3*c^3*d^3}
...
TEST ///
-- test 9 : DGAlgebra ** DGAlgebra - need to add in an assert
R = ZZ/101[a,b,c,d]
I = ideal(a,b)
J = ideal(c,d)
A = koszulComplexDGA(I)
...
TEST ///
-- test 10 : isHomologyAlgebraTrivial, getGenerators, findTrivialMasseyOperation
Q = ZZ/101[x_1..x_6]
I = ideal (x_3*x_5,x_4*x_5,x_1*x_6,x_3*x_6,x_4*x_6)
R = Q/I
A = koszulComplexDGA(R)
...
homList = apply(5,i -> prune homology(i,A))
-- need to check this!
homologyAlgebra(A)
