...
xmlAttributes(localInterpState:threadLocalInterp,e:Expr):Expr := (
...
xmlParse(localInterpState:threadLocalInterp,e:Expr):Expr := (
...
xmlGetChildren(localInterpState:threadLocalInterp,e:Expr):Expr := (
     when e
...
xmlGetName(localInterpState:threadLocalInterp,
e:Expr):Expr := (
     when e
...
xmlGetContent(localInterpState:threadLocalInterp,e:Expr):Expr := (
     when e
...
xmlGetNext(localInterpState:threadLocalInterp,e:Expr):Expr := (
     when e
...
xmlIsElement(localInterpState:threadLocalInterp,e:Expr):Expr := (
     when e
     is node:xmlNode do toExpr(isElement(node))
     else WrongArg("an XML node"));
setupfun("xmlIsElement",xmlIsElement);
xmlIsText(localInterpState:threadLocalInterp,e:Expr):Expr := (
     when e
     is node:xmlNode do toExpr(isText(node))
     else WrongArg("an XML node"));
setupfun("xmlIsText",xmlIsText);

xmlNewDoc(localInterpState:threadLocalInterp,e:Expr):Expr := (
     when e is s:string do Expr(NewDoc("1.0",s))
     else WrongArgString());
setupfun("xmlNewDoc",xmlNewDoc);

xmlNewProp(localInterpState:threadLocalInterp,e:Expr):Expr := (
...
xmlNewChild(localInterpState:threadLocalInterp,e:Expr):Expr := (
...
xmlNewText(localInterpState:threadLocalInterp,e:Expr):Expr := (
     when e is args:Sequence do if length(args) != 2 then WrongNumArgs(3) else
     when args.0 is parent:xmlNode do
