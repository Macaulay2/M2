...
mapFromExceptionalCurves (EmbeddedProjectiveVariety,Sequence) := o -> (U,dab) -> (
    ...
    if (not withNorm) and d == 1 and a == 0 and b == 0 then return multirationalMap super toRationalMap(1_U);
    toNormU := null;
    if withNorm then (
    ...
    if o#"IsK3Type" and o#"TargetExpDim" =!= null then (
...
buildContractionMapCastelnuovo = (U,X,Verb) -> (
    ...
    if member(recognize X,{"planeInPP7", "internal-projection-K3-genus-8", "surf-5-6-2-nodal"}) then (
        f = mapFromExceptionalCurves(U,(1,0,0),Verbose=>Verb,"Normalization"=>false,"IsK3Type"=>false);
        return saveAndReturnMap(f,U);
    );
    if recognize X === "surf-5-7-0-1" then (
        f = mapFromExceptionalCurves(U,(1,1,0),Verbose=>Verb,"Normalization"=>false,"IsK3Type"=>false);
        return saveAndReturnMap(f,U);
    );
    if recognize X === "surf-4-3-1-external" then (
        f = mapFromExceptionalCurves(U,(1,0,0),Verbose=>Verb,"Normalization"=>true,"IsK3Type"=>false);
        return saveAndReturnMap(f,U);
    );
...
buildContractionMapK3 = (U,X,Str,Verb) -> (
    ...
    if recognize X === 3 then (
        f = mapFromExceptionalCurves(U,(1,0,0),Verbose=>Verb,"Normalization"=>true,"TargetExpDim"=>genK3);
        return saveAndReturnMap(f,U);
    );
    if member(recognize X,{"oneNodalSepticDelPezzoSurfaceC26", 18}) then (
        ...
        f = mapFromExceptionalCurves(U,(1,1,degree C),Verbose=>Verb,"Normalization"=>true,"ForceExperimentalNormalization"=>false,"TargetExpDim"=>genK3);
        return saveAndReturnMap(f,U);
    );
    if recognize X === 1 then (
        f = mapFromExceptionalCurves(U,(1,1,degree C),Verbose=>Verb,"Normalization"=>false);
    ...
    if recognize X === "C42" then (
        f = mapFromExceptionalCurves(U,(1,1,2),Verbose=>Verb,"Normalization"=>false,"TargetExpDim"=>genK3);
        return saveAndReturnMap(f,U);
    );
    if recognize X === "mukai26''" then (
        f = mapFromExceptionalCurves(U,(1,1,3),Verbose=>Verb,"Normalization"=>false,"TargetExpDim"=>genK3);
        return saveAndReturnMap(f,U);
    );
    if recognize X === "FarkasVerra" then (
        ...
        n := inverseNormalizationMapRaw(image f,"f(U)",2,Verb);
        f = super(f * n);
        assert(f#"image" =!= null);
        return saveAndReturnMap(f,U);
    );
    if euler hilbertPolynomial U == 2 then (
        f = mapFromExceptionalCurves(U,(1,1,degree C),Verbose=>Verb,"Normalization"=>false,"TargetExpDim"=>genK3);
        return saveAndReturnMap(f,U);
    );
    -- cases included: "NotRecognized", "october2021-34'", "october2021-20"
    if Verb then << "-- skipping computation of the map f from U to the minimal K3 surface of degree " << discriminant X << endl;
);

buildContractionMapK3DSCF = (U,X,withNorm,Str,Verb) -> (
    if U.cache#?"MapToMinimalK3Surface" then return U.cache#"MapToMinimalK3Surface";
    local f;
    if isStandardK3surface U then (
        if Verb then << "-- U is already in the target space; defining f as the identity map" << endl;
        f = mapFromExceptionalCurves(U,(1,0,0),Verbose=>false,"Normalization"=>false);
        return U.cache#"MapToMinimalK3Surface" = f;
    );
    if (not withNorm) and euler hilbertPolynomial U != 2 then return;
    b := null;
    (L,C) := U.cache#"exceptionalCurves";
    if dim C == -1 or isHigherDegreeCurveInExceptionalSetKnownToBeSpecial(X) then b = 0;
    if dim C == 1 and sectionalGenus C == 0 then b = degree C;
    if b === null then (
        if Verb then << "-- warning: multiple exceptional curves of degree > 1 detected; not yet supported" << endl;
        return;
    );
    f = mapFromExceptionalCurves(U,(1,1,b),Verbose=>Verb,Strategy=>Str,"Normalization"=>withNorm,"TargetExpDim"=>(if dim C == -1 then sectionalGenus U else null));
    return saveAndReturnMap(f,U);
);
