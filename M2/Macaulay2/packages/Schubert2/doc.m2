Node
  ...
  Description
    Text
      ...
      of dimension zero (integration), and the Chern character of its tangent bundle (if
      known).  The intersection ring is represented as a {\em Macaulay2} @ TO2{Ring,"ring"} @, and its elements are {\em
      Macaulay2} @ TO2{RingElement,"ring elements"} @. (More precisely, the intersection ring used by Schubert2 may be
      only the subalgebra of the Chow ring tensored with the rationals generated by some explicit classes.)
      
      An @TO2{AbstractSheaf,"abstract sheaf"}@ on an abstract variety is represented by an object of class 
      @ TO AbstractSheaf @ that contains the total Chern character of the sheaf in the intersection ring.
      It should be thought of as an element of
      the Grothendieck ring of the variety tensored with the rational numbers, which, by the
      Grothendieck-Riemann-Roch theorem, is isomorphic to the Chow ring tensored with the rationals; the isomorphism is provided
      ...
      This package is inspired by @ HREF{"https://jmokland.github.io/schubert/", "schubert"} @, which was written by
	   @ HREF{"https://katzs.web.illinois.edu/", "Sheldon Katz"} @
	   and @ HREF{"https://mathshistory.st-andrews.ac.uk/Biographies/Stromme/", "Stein A. Strømme"} @.
      ...
      In 2026, functions for weighted projective spaces and symmetric and skew-symmetric degeneracy loci
      were added.
...
Node
  ...
  Inputs
    ...
    VariableName => Symbol
      the symbol to use for the variable representing the first Chern class of the tautological line bundle $O(1)$ on the resulting projective space
  Outputs
    :
...
Node
  ...
  Description
    Text
      If $X$ is an abstract variety and its tautological line bundle {\tt OO_X(1)} has already been built,
...
Node
  Key
    ...
    [weightedProjectiveBundle,VariableName]
  Headline
  ...
  Synopsis
    ...
    Outputs
      :
        the weighted projective space $\PP^n(a_0,\ldots,a_n)$,
        given a list of positive integer weights $\{ a_0,\ldots,a_n\}$.
    Description
      Text
        A weighted projective space is viewed as a smooth Deligne-Mumford stack, not as a singular variety.
	Explicitly, $W = \PP^n(a_0,\ldots,a_n)$ is the quotient stack $[(A^{n+1}-0)/G_m]$,
	where the multiplicative group $G_m$ acts on affine space by $t(x_0,\ldots,x_n)=(t^{a_0}x_0,\ldots,t^{a_n}x_n)$.
        Since $W$ is smooth, the tangent bundle $TW$ is a vector bundle on $W$, and Schubert2 can make computations
	with its Chern classes. Explicitly, the Euler sequence for $W$ has the form
        $$0\to O_W\to \oplus_{i=0}^n O_W(a_i)\to TW\to 0.$$
	The intersection ring of $W$ is $\mathbf{Q}[h]/(h^{n+1})$, where $h = c_1(O_W(1))$,
        as with the usual projective space; but the degree of the zero-cycle $h^n$ is $1/(a_0\ldots a_n)$ rather than 1.
      Text
        Note that the Schubert2 function @TO2 {(chi,AbstractSheaf),"chi"} @
        uses the Hirzebruch-Riemann-Roch formula to compute Euler characteristics
	of vector bundles, and that gives meaningless answers on a stack such as $W$.
	Likewise for @TO2 {(euler,AbstractVariety),"euler W"} @. However, functions such
        as @TO sectionZeroLocus@, @TO degeneracyLocus@, and @TO skewDegeneracyLocus@ can
        yield subvarieties $Y$ of $W$ that miss the points with nontrivial stabilizer group,
        and then @TO2 {(chi,AbstractSheaf),"chi"} @ on $Y$ gives meaningful results.
      Text
        For example, consider the weighted projective space $W = \PP^7(1^7,2)$.
        We can define
        a smooth Fano 4-fold $Y$ in $W$ by 5 Pfaffian equations (the locus where a general skew-symmetric map $E\to E^*\otimes L$
        has rank at most 2, in terms of the vector bundles below). We compute that $Y$ has degree $(-K_Y)^4 = c_1(O_Y(1))^4 = 10.
  ...
  Synopsis
    Usage
      weightedProjectiveBundle weights
      weightedProjectiveBundle(weights,X)
    Inputs
      weights : List
      X : AbstractVariety
      VariableName => Thing
    Outputs
      :
        the product $W = \PP^n(a_0,\ldots,a_n) \times X$,
        given a list of positive integer weights $\{ a_0,\ldots,a_n\}$.
        If $X$ is omitted, then @ TO point @ is used as the base. The projection morphism $W\to X$
        is given in Schubert2 by the notation $W/X$ (with whatever names you choose for these varieties).
  SeeAlso
   projectiveBundle
   abstractProjectiveSpace
...
Node
  ...
  Description
   Text
    Maybe we should remove this operation, in favor of @ TO (symbol *, ZZ, AbstractSheaf) @. As indicated below,
    Schubert2 interprets (OO_X(1))^10 as a vector bundle of rank 10, whereas OO_X(1)^10 is the line bundle OO_X(10).
    So it is probably wiser to write 10*OO_X(1) when you want the bundle of rank 10.
   Example
    X = abstractProjectiveSpace 1
    F = OO_X(1)
    F^10
...
Node
  ...
  Outputs
   :
    ...
    the {\tt n}-th (tensor) power of the tautological line bundle $O_X(1)$ on the variety $X$ of {\tt F}.
...
Node
 Key
  ...
  sectionZeroLocus
 Headline
  the zero locus of a general section of a vector bundle, as an abstract variety
 Usage
  sectionZeroLocus F
 Inputs
  F:
 ...
 Description
  Text
    If you just want the cycle class of the zero locus, that can be obtained by the function @TO2 {(ctop,AbstractSheaf),"ctop"} @.
  Example
...
Node
 Key
  ...
  degeneracyLocus2
 Headline
  the class of a rank locus for a general map between vector bundles
 Usage
 ...
 Description
  Text													 
   It is assumed that there are enough such maps that the locus has the expected dimension. Namely,
   in terms of $a := \text{rank}(A)$ and $b := \text{rank}(B)$, the class is in codimension $(a-k)(b-k)$.
   The formula for this class was given by Porteous.
   The degeneracy locus as an abstract variety
   can be obtained by @TO degeneracyLocus@.
  Example
 ...
 SeeAlso
  degeneracyLocus
  kernelBundle
  symmetricDegeneracyLocus2
  skewDegeneracyLocus2
  ctop
...
Node
 Key
  ...
  degeneracyLocus
 Headline
  a rank locus for a general map between vector bundles, as an abstract variety
 Usage
 ...
 Description
  Text
   More precisely, the function yields an abstract variety $S$ that is a resolution of singularities of the locus $S_0$
   where a general symmetric map $A \to B$ has rank at most $k$,
   following Fulton's Intersection Theory, section 14.4.
  Text
   It is assumed that there are enough such maps that $S_0$ has the expected dimension. Namely,
   in terms of $a := \text{rank}(A)$ and $b := \text{rank}(B)$, this locus is in codimension $(a-k)(b-k)$.
   If the map $A \to B$ never has rank less than $k$ on the base variety $X$, then $S$ is isomorphic
   to the actual degeneracy locus $S_0$. The class of the degeneracy locus
   in the intersection ring of $X$ can be obtained by @TO degeneracyLocus2@.
  Example
   X = base(5, Bundle => (A,3,a), Bundle => (B,3,b))
...
Node
 Key
  ...
  kernelBundle
 Headline
  the kernel bundle on a rank locus of a general map between vector bundles
 Usage
  kernelBundle(k,B,A)
 Inputs
  k:
 ...
 Outputs
  :
   the kernel bundle on the locus where the rank of a general map from {\tt A} to {\tt B} is at most {\tt k}, assuming
   that the rank doesn't drop further
 Description
  Text
   More precisely, the function yields a vector bundle of rank $\rank(A)-k$ on
   @TO2{(degeneracyLocus,ZZ,AbstractSheaf,AbstractSheaf),"degeneracyLocus(k,B,A)"}@,
   which is a resolution of singularities of the locus $S_0$
   where a general map $A \to B$ has rank at most $k$.
  Example
   X = base(5, Bundle => (A,3,a), Bundle => (B,3,b))
 ...
 SeeAlso
  degeneracyLocus
  symmetricKernelBundle
  skewKernelBundle

Node
 Key
  (symmetricDegeneracyLocus2,ZZ,AbstractSheaf,AbstractSheaf)
  (symmetricDegeneracyLocus2,ZZ,AbstractSheaf)
  symmetricDegeneracyLocus2
 Headline
  the class of a rank locus for a general symmetric map 
 Usage
  symmetricDegeneracyLocus2(k,E,L)
 Inputs
  k:
  E:
  L:
 Outputs
  :
   the cycle class of the locus where the rank of a general symmetric map from $E$ to $E^*\otimes L$ is at most $k$.
 Description
  Text
   The function yields the class in the intersection ring of the locus
   where a general symmetric map $E \to E^*\otimes L$ has rank at most $k$.
   Here $E$ is a vector bundle and $L$ is a line bundle, on an abstract variety $X$.
   If $L$ is omitted, it is taken to be the trivial line bundle.
  Text													 
   It is assumed that there are enough such maps that the locus has the expected dimension. Namely,
   in terms of $d := \text{rank}(E)-k$, the class is in codimension $\binom{d+1}{2}$.
   The formula for this class was given by Harris-Tu and J\'{o}zefiak-Lascoux-Pragacz.
   The degeneracy locus as an abstract variety
   can be obtained by @TO symmetricDegeneracyLocus@.
  Text
   For example, consider the locus where a $3\times 3$ symmetric matrix (up to scalars)
   has rank at most 1.
   Geometrically, this is the Veronese embedding of $\PP^2$ in $\PP^5$ via the line bundle $O(2)$,
   so it is a surface of degree 4 in $\PP^5$. Let us check that.
  Example
   X = abstractProjectiveSpace 5;
   E = 3*OO_X;
   L = OO_X(1);
   h = chern_1(OO_X(1));
   locus1 = symmetricDegeneracyLocus2(1,E,L);
   integral(h^2*locus1)
  Text
   Next, consider the locus where a $3\times 3$ symmetric matrix (up to scalars) has rank at most 2.
   This is given by the vanishing of the determinant, so it is a cubic 4-fold in $\PP^5$.
   Let us check that.
  Example								 
   locus2 = symmetricDegeneracyLocus2(2,E,L);
   integral(h^4*locus2)
 SeeAlso
  symmetricDegeneracyLocus
  degeneracyLocus2
...
Node
 Key
  (symmetricDegeneracyLocus,ZZ,AbstractSheaf,AbstractSheaf)
  (symmetricDegeneracyLocus,ZZ,AbstractSheaf)
  symmetricDegeneracyLocus
 Headline
  a rank locus for a general symmetric map, as an abstract variety
 Usage
  symmetricDegeneracyLocus(k,E,L)
 Inputs
  k:
  E:
  L:
 Outputs
  :
   the locus where the rank of a general symmetric map from $E$ to $E^*\otimes L$ is at most $k$.
 Description
  Text
   More precisely, the function yields an abstract variety $S$ that is a resolution of singularities of the locus $S_0$
   where a general symmetric map $E \to E^*\otimes L$ has rank at most $k$,
  ...
  Text
   It is assumed that there are enough such maps that $S_0$ has the expected dimension. Namely,
   in terms of $d := \text{rank}(E)-k$, this locus is in codimension $\binom{d+1}{2}$.
   If the map $E \to E^* \otimes L$ never has rank less than $k$ on $X$, then $S$ is isomorphic
   to the actual degeneracy locus $S_0$. The class of the degeneracy locus
   in the intersection ring of $X$ can be obtained by @TO symmetricDegeneracyLocus2@.
  Text
   For example, consider the locus $W$ where a $3\times 3$ symmetric matrix (up to scalars)
   has rank at most 1.
   Geometrically, this should be the Veronese embedding of $\PP^2$ in $\PP^5$ via the line bundle $O(2)$,
   which is a surface of degree 4 in $\PP^5$. Let us check that, and also
   that $\chi(\PP^2,T\PP^2) = 8$.
  Example
   X = abstractProjectiveSpace 5;
   E = 3*OO_X;
   L = OO_X(1);
   h = chern_1(OO_X(1));
   S = symmetricDegeneracyLocus(1,E,L)
   chi(tangentBundle S)
   f = S/X
   locus1 = f_*(1);
   integral(h^2*locus1)
...
Node
 Key
  (symmetricKernelBundle,ZZ,AbstractSheaf,AbstractSheaf)
  symmetricKernelBundle
 Headline
  the kernel bundle on a rank locus of a general symmetric map
 Usage
  symmetricKernelBundle(k,E,L)
 Inputs
  k:
  E:
  L:
 Outputs
  :
   the kernel bundle on the locus where the rank of a general symmetric map from $E$ to $E^*\otimes L$ is $k$
 Description
  Text
   More precisely, the function yields a vector bundle of rank $\rank(E)-k$ on
   @TO2{(symmetricDegeneracyLocus,ZZ,AbstractSheaf,AbstractSheaf),"symmetricDegeneracyLocus(k,E,L)"}@,
   which is a resolution of singularities of the locus $S_0$
   where a general symmetric map $E \to E^*\otimes L$ has rank at most $k$.
   Here $E$ is a vector bundle and $L$ is a line bundle, on an abstract variety $X$.
   If $L$ is omitted, it is taken to be the trivial line bundle.
  Text
   ...
   has rank at most 1.
   Geometrically, this is the Veronese embedding of $\PP^2$ in $\PP^5$ via the line bundle $O(2)$,
   which is a surface of degree 4 in $\PP^5$. Viewing this surface as $\PP^2$, the kernel bundle $K$
   is the kernel of the natural map $O_{\PP^2}^3\to O_{\PP^2}(1)$ on $\PP^2$.
   Let us check that $\chi(\PP^2,K)=0$ and $\chi(\PP^2,K^*)=3$.
  Example
   X = abstractProjectiveSpace 5;
   E = 3*OO_X;
   L = OO_X(1);
   h = chern_1(OO_X(1));
   K = symmetricKernelBundle(1,E,L)
   chi(K)
   chi(dual(K))
 SeeAlso
   kernelBundle
...
Node
 Key
  (skewDegeneracyLocus2,ZZ,AbstractSheaf,AbstractSheaf)
  (skewDegeneracyLocus2,ZZ,AbstractSheaf)
  skewDegeneracyLocus2
 Headline
  the class of a rank locus for a general skew-symmetric map 
 Usage
  skewDegeneracyLocus2(k,E,L)
 Inputs
  k:
  E:
  L:
 Outputs
  :
   the cycle class of the locus where the rank of a generic skew-symmetric map from $E$ to $E^*\otimes L$ is at most $k$.
 Description
  Text
   In other words, this is a locus defined by Pfaffian equations.
   The function yields the class in the intersection ring of the locus
   where a general skew-symmetric map $E \to E^*\otimes L$ has rank at most $k$.
   The number $k$ must be even. Here $E$ is a vector bundle and $L$ is a line bundle, on an abstract variety $X$.
   If $L$ is omitted, it is taken to be the trivial line bundle.
  Text													 
   It is assumed that there are enough such maps that the locus has the expected dimension. Namely,
   in terms of $d := \text{rank}(E)-k$, the class is in codimension $\binom{d}{2}$.
   The formula for this class was given by Harris-Tu and J\'{o}zefiak-Lascoux-Pragacz.
   The degeneracy locus as an abstract variety
   can be obtained by @TO skewDegeneracyLocus@.
  Text
   For example, consider the locus where a $6\times 6$ skew-symmetric matrix (up to scalars)
   has rank less than 6 (hence at most 4). This is the cubic hypersurface in $\PP^{14}$ defined by the Pfaffian.
   Let us check that.
  Example
   X = abstractProjectiveSpace 14;
   E = 6*OO_X;
   L = OO_X(1);
   h = chern_1(OO_X(1));
   locus4 = skewDegeneracyLocus2(4,E,L);
   integral(h^(13)*locus4)
  Text
   Next, consider the locus where a $6\times 6$ skew-symmetric matrix (up to scalars) has rank at most 2.
   This is the Grassmannian $\text{Gr}(2,6)$ of lines in $\PP^5$; it is an 8-fold of degree 14 in $\PP^{14}$.
   Let us check that.
  Example								 
   locus2 = skewDegeneracyLocus2(2,E,L);
   integral(h^8*locus2)  
 SeeAlso
  skewDegeneracyLocus
  degeneracyLocus2
...
Node
 Key
  (skewDegeneracyLocus,ZZ,AbstractSheaf,AbstractSheaf)
  skewDegeneracyLocus
 Headline
  a rank locus for a general skew-symmetric map, as an abstract variety
 Usage
  skewDegeneracyLocus(k,E,L)
 Inputs
  k:
  E:
  L:
 Outputs
  :
   the locus where the rank of a generic skew-symmetric map from $E$ to $E^*\otimes L$ is at most $k$.
 Description
  Text
   In other words, this is a locus defined by Pfaffian equations.
   More precisely, the function yields an abstract variety $S$ that is a resolution of singularities of the locus $S_0$
  ...
  Text													 
   It is assumed that there are enough such maps that $S_0$ has the expected dimension. Namely,
   in terms of $d := \text{rank}(E)-k$, this locus is in codimension $\binom{d}{2}$.
   If the map $E \to E^* \otimes L$ never has rank less than $k$ on $X$, then $S$ is isomorphic
   to the actual degeneracy locus $S_0$.
   The class of the degeneracy locus
   in the intersection ring of $X$ can be obtained by @TO skewDegeneracyLocus2@.
  Text
   For example, consider the locus $W$ where a $6\times 6$ skew-symmetric matrix (up to scalars)
   has rank at most 2.
   This is the Grassmannian $\text{Gr}(2,6)$ of lines in $\PP^5$; it is an 8-fold of degree 14 in $\PP^{14}$.
   Let us check this, and also that $\chi(\text{Gr}(2,6),T\text{Gr}(2,6)) = 35$.
  Example
   X = abstractProjectiveSpace 14;
   E = 6*OO_X;
   L = OO_X(1);
   h = chern_1(OO_X(1));
   W = skewDegeneracyLocus(2,E,L)
   chi(tangentBundle W)
   f = W/X
   locus2 = f_*(1);
   integral(h^8*locus2)  
...
Node
 Key
  (skewKernelBundle,ZZ,AbstractSheaf,AbstractSheaf)
  skewKernelBundle
 Headline
  the kernel bundle on a rank locus of a general skew-symmetric map
 Usage
  skewKernelBundle(k,E,L)
 Inputs
  k:
  E:
  L:
 Outputs
  :
   the kernel bundle on the locus where the rank of a general skew-symmetric map from $E$ to $E^*\otimes L$ is $k$
 Description
  Text
   More precisely, the function yields a vector bundle of rank $\rank(E)-k$ on
   @TO2{(skewDegeneracyLocus,ZZ,AbstractSheaf,AbstractSheaf),"skewDegeneracyLocus(k,E,L)"}@,
   which is a resolution of singularities of the locus $S_0$
   where a general skew-symmetric map $E \to E^*\otimes L$ has rank at most $k$. The number $k$ must be even.
   Here $E$ is a vector bundle and $L$ is a line bundle, on an abstract variety $X$.
   If $L$ is omitted, it is taken to be the trivial line bundle.
  Text
   For example, consider the locus where a $6\times 6$ skew-symmetric matrix (up to scalars)
   has rank at most 2.
   This is the Grassmannian $W = \text{Gr}(2,6)$ of lines in $\PP^5$. The kernel bundle $K$ is the dual of the rank-4
   quotient bundle $Q$, $0\to S\to O_W^6\to Q\to 0$ over the Grassmannian. Let us check that $\chi(W, K)=0$
   and $\chi(W,dual(K))=6$.
  Example
   X = abstractProjectiveSpace 14;
   E = 6*OO_X;
   L = OO_X(1);
   K = skewKernelBundle(2,E,L)
   chi(K)
   chi(dual(K))
 SeeAlso
   kernelBundle
...
Node
  ...
  Description
    Text
      Returns the tautological line bundle of the variety $X$, also called {\tt OO_X(1)}.  In the case of a
