...
seedTest (PolySystem, Point, Point):= o -> (P, p0, x0) -> (
    ...
    *-
	...
	J := evaluate(jacobian Pflat,xflat);
    	dimFp := numericalRank(J_(toList(0..n-1)), Threshold => o.Threshold);
    	m' := m+n-numericalRank(J, Threshold => o.Threshold);
	n' := numericalRank(J_(toList(n..n+m-1)), Threshold => o.Threshold);
	if (n>dimFp) then (
    	    passed.ZDIM = false;
	    if o.Verbose then << "ZDIM fails: local dimension = " << n' << " in ambient space of dim " << n << endl;
	    )
    	else (
	    passed.ZDIM = true;
	    if (m>m') then (
...
-- test for sparseSolver which sometimes fails
setRandomSeed 0
S = QQ[x,y]
...
coreMonodromySolve (HomotopyGraph, HomotopyNode) := o -> (HG,node1) -> (
    ...
    if o.ReturnSeed then (node1, npaths) else (lastNode, npaths)
)
