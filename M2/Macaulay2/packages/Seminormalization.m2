...
isSeminormal(Ring) := R1 -> (
    ...
    if (dim coker myMap >= 0) then (return false;) else (return true;);
);
...
seminormalize(Ring) := o->R1 -> (
    ...
    return {target renamedVarsList#0, renamedVarsList#0, (seminormalizedList#2)*(renamedVarsList#2)};    
);

internalSeminormalize = method( Options => { Variable => symbol Yy, Strategy => AllCodimensions  });

internalSeminormalize(Ring) := o -> R1 -> (
    --print "starting internalSeminormalize";
    seminormalizationRecursive(R1, 1, Strategy=>o.Strategy)
);

--internal functions 

--FIND ME. Standardize tabbing in this area. it looks weird that some of it is further than others. Also decide on how commenting should look, IE
 {*
    ACTUAL CODE HERE TM
    --COMMENT STYLE 1 (means less effort when uncommenting and commenting. also an even line looks nice)
  --COMMENT STYLE 2 (more obvious that something is commented)
 *}
 --pick one of them and stick with it.
--*****************************************
...
seminormalizationRecursive(Ring, ZZ):=o->(inputRing, inputInteger)->(
    ...
    *}
    ...
    if dim(inputRing)<=0 then(
        if (o.Verbose == true) then print "seminormalizationRecursive: Returning because input ring has dimension 0.";
    ...
    if(conductorIdeal===ideal(1_inputRing)) then{
        if (o.Verbose == true) then print "seminormalizationRecursive: Returning because input ring is normal.";
        return {normalizedInputRing, normalizationMapOfInputRing, map(normalizedInputRing, normalizedInputRing)};
    };

    if (o.Verbose == true) then print "The input ring (or recursive input ring) was not already seminormalized";
    ...
    if (o.Verbose == true) then print "checkpoint 1";

    --conductorIdealInNorm:=normalizationMapOfInputRing(conductorIdeal); --send the conducor ideal through the normalization map
    --conductorIdealR:=conductorIdealInNorm*normalizedInputRing;--product of conductor ideal and the normalized ring

    --conductorIdealR:=(normalizationMapOfInputRing(conductorIdeal))*normalizedInputRing;--send the conductor ideal through the normalization map and then apply it to the normalized ring
    --this would be IR^N
    --radicalOfConductorR:=radical(conductorIdealR); --again, radical not the best
    --rad(IR^N)
    ...
    if (o.Verbose == true) then print "checkpoint 2";
    ...
    if (o.Verbose == true) then print "checkpoint 3";


    if (o.Verbose == true) then print "input to the recursion is";
    if (o.Verbose == true) then print inputRingModRadical;
    recursionOutput := seminormalizationRecursive(inputRingModRadical, inputInteger+2, Strategy=>o.Strategy, Verbose=>o.Verbose, Variable=>o.Variable);--recurse. inputInteger goes up by 2. inputInteger+1 is used below. Should allow the user to track where stuff is renamed (at least a little)
    ...
    if (o.Verbose == true) then print "checkpoint 4";
    ...
    --ughRing2 := flattenVarDegrees(source mapPsi);
    ...
    --flatten the source of mapPsi and then remap it to the target. target is given by mapPhi, because we already sent that one to the right place and they go to the same place.
	  if (o.Verbose == true) then print "checkpoint 5";
	----******MAKE SURE EVERYTHING IS FLATTENED BEFORE PULLING BACK (we haven't done this yet...)
      if (o.Verbose == true) then (
          print "We are about to do a pullback.  Normalization to conductor is";
          print mapPhi;
          print "gluing map is";
          print mapPsi;
      );
	  ...
	  if (o.Verbose == true) then (
	    print "Here is the pullback output";
	    print temp;
	  );
	  
	  --now we need to reconstruct the map from inputRing to RSN = temp#0
    RSNtoRN := temp#1;--the map from Semi(R) to R^N
    RVarsInRN := apply(first entries vars inputRing, xy -> normalizationMapOfInputRing(xy));
...
listOfVariableCreator=(inputList, inputInteger)->(
    outputList:={};
    lengthOfList:=length inputList;
    lengthOfList=lengthOfList-1;
    for count from 0 to lengthOfList do outputList=append(outputList, concatenate(replace("[^[:alnum:]]+", toString(count), toString(inputList#count)), "RE", toString(inputInteger)));
    return outputList;
)
...
ringRenamerAndMap(Ring, ZZ):=(inputRing, inputInteger)->(
	currentList:=first entries vars ambient inputRing;
    nextList:=listOfVariableCreator(currentList, inputInteger);
    outputRing:=newRing(inputRing, Variables=>nextList);
	additionalList:=first entries vars ambient outputRing;
    return {outputRing, map(outputRing, inputRing, additionalList), map(inputRing, outputRing, currentList)};
);
...
renameVariablesOfTarget(Symbol, RingMap) := (varName, mapR1toR2) -> (
    R2 := target mapR1toR2;
    newVarList := apply(#(first entries vars R2), tt -> varName_tt);
    newSeminormalization := newRing(R2, Variables=>newVarList);
    gMap := map(newSeminormalization, R2, first entries vars newSeminormalization);
    hMap := map(R2, newSeminormalization, first entries vars R2);
    return (gMap*mapR1toR2, gMap, hMap);
);

--one more way to do this, this renames the variables, then returns the maps both ways
varRenamer = method();
varRenamer(String, Ring) := (varName, R1) -> (
    newVarList := apply(#(first entries vars R1), tt -> (varName | toString(tt)) );
    R1A := newRing(R1, Variables=>newVarList);
    mapTo := map(R1A, R1, first entries vars R1A);
    mapFrom := map(R1, R1A, first entries vars R1);
    return {R1A, mapTo, mapFrom};
);
...
--We use an elimination order to accomplish this
    ...
    S1 := target(phi);
    newVar := local newVar;
    R2 := coefficientRing(R1)[{newVar} | (first entries vars ambient R1), MonomialOrder=>Eliminate 1];
    phi2 := map(S1, R2, {gg} | apply(first entries vars R1, xx -> phi(xx)));
    K2 := ker phi2;
...
--We tensor the two rings together.  Then line up minimal primes (if necessary)
    ...
    minPrimeListBigBC := apply(minimalPrimes(ideal BigBC), q -> sub(q, BigBC)); --thus we grab the minimal primes of BigBC
    minPrimeListC := apply(minimalPrimes(ideal flatC), q -> sub(q, flatC)); --and the minimal primes of B
    ...
    --prime list of B each time it finds a match.  When B runs out of primes, the loop should exit.
    --newPrimeList should store the list of the minimal primes of 
    while ((#minPrimeListC > 0) and (#minPrimeListBigBC > 0)) do (
        ...
        i = 0;
        flag = true;
        while( (flag == true) and (i < #minPrimeListC) ) do(
            if (invPrime == minPrimeListC#i) then ( --check if that pulled back prime is in minPrimeList for B
                flag = false;
                newPrimeList = append(newPrimeList, minPrimeListBigBC#0); --add it to the newPrimeList
                minPrimeListC = drop(minPrimeListC, {i,i});                
            );
            i = i+1
        );
        if (flag == true) then error "findOverring: The tensor product has too many minimal primes.  Are you sure both maps are integral?";
    ...
    newZero := intersect(newPrimeList); --this is the new zero ideal of our big ring.
    return {(flattenRing(BigBC/newZero))#0, newf, newg, map(flatB, target f, sub(matrix rB#1, flatB)), map(flatC, target g, sub(matrix rC#1, flatC))};
    --this returns to overring, 
    --the map A->newlabeledB,
    --the map A->newlabeledC,
    --the map oldB -> newlabeledB (an iso)
    --the map oldC -> newlabeledC (an iso)
);
...
--    A to A[YY1, ..., YY_n]/I, variables go to variables.
	A:=source ff;
	B:=target ff;
	currentListB:=first entries vars ambient B;    --get the variables of B
	nextListB:=listOfVariableCreator(currentListB, nn); --rename the variables of B (using the nn given to avoid collisions)
	currentListA:=first entries vars ambient A; --get the variables of A (we leave these alone)
    ...
    return {newB, BtoNewB}; --return the A algebra A[...]/? and the map B->A[...]/? (a ring isomorphism)
);
...
flattenVarDegrees(Ring) := (R1) -> (
    ...
    varList := first entries vars ambient R1;
...
--and finally view that map as a ring.
    ...
    --this returns first
    --1. the map from source ring to the intersection
    --2. the map from the intersection to B (= target of gg)
);
...
--we do this by keeping track of idempotents
    ...
    varImagesInQuotients := apply(#quotientDomainList, jj -> apply(first entries vars R1, vv -> (mapR1ToQuotientList#jj)(vv) ) );
    ...
    return (renamedNorm#1)*(nearlyFinalMap);
);
...
--   e') (note this implies e_i*x_{i', j} = 0)
    ...
    if (#listRings == 1) then return {listRings#0, sub(1, listRings#0), first entries vars (listRings#0)};
	...
	numRings:=#listRings-1;

    --let's make the variables for our new ring
	for count from 0 to numRings do { 
		newListRings=append(newListRings, ringRenamer(ringRenamer(listRings#count, count), count)); 
		    --first we make a new list of rings, with variables renamed to avoid collisions
		curVars:=first entries vars ambient (newListRings#count);
		    --grab the variables (with the new names)
		listVars=append(listVars, curVars);
		    --make a list of all the variables (well, a list of lists)
		idems=append(idems, "e"|count);
		    --make a list of idempotents
	};

	moreCompleteList:={};

	for count from 0 to numRings do { --let's create a list with the idempotents and the ring variables 
	                                  --(flattened this time).	                                  
		moreCompleteList=(moreCompleteList|(listVars#count));
		moreCompleteList=append(moreCompleteList, idems#count);
	};

	unQuotiented := myCoeffRing[moreCompleteList]; --make the ring out of all these variables

	totalVars := first entries vars unQuotiented; --get the variable names of our new ring
	...
	for count from 0 to numRings do {
		...
		listOfIdems=append(listOfIdems, modVarList#0);
		modVarList=take(modVarList, -(#modVarList-1));
		listOfListsOfVars=append(listOfListsOfVars, tempList);
	};
	...
	for count from 0 to numRings do {
		for count1 from count to numRings do {
		    --we run two loops
	...
	megaIdeal = megaIdeal + ideal( sum(listOfIdems) - 1); --add these idempotent relations to our big ideal
	...
	if (o.Variable === null) then (
	    ...
	    hMap = renamedProd#1;
	    return {renamedProd#0, apply(subbedIdempotents, ei -> hMap(ei)), apply(subbedRingVarTargets, li -> apply(li,  ti -> hMap(ti))) };
    )	    
...
doc ///
    ...
    Description
        Text
            This turns a multigraded ring into a singly graded ring, summing the individuall degrees.
        Example
...
doc ///
    Key
        renameVariablesOfTarget
        (renameVariablesOfTarget, Symbol, RingMap)
    Headline
        given a ring map, it renames the variables of the target to the specified name
    Usage
        L = renameVariablesOfTarget(mySymb, f)
    Inputs
        mySymb: Symbol
        f: RingMap
    Outputs
    ...
    Description
        ...
        Example
            A = QQ[a,b]/ideal(a^2-b^5);
            B = QQ[u,v, w];
            f = map(A, B, {a^2, a*b, b^2});
            L = renameVariablesOfTarget(X, f);
            L#0
            L#1
            L#2
///
