...
isSeminormal(Ring) := R1 -> (
    ...
    M := pushFwd(R1toR1SN);
    myMap := pushforward(M, sub(1, target R1toR1SN));
    isSurjective myMap
);
...
mapModules(RingMap) := RingMapInput ->(
    ...
    pushforward(actualMod, sub(1, target RingMapInput))
)
...
--and finally view that map as a ring.
	...
	ASNoverA := pushFwd(f); --likewise ASN
	BoverA := pushFwd(g); --and B as A modules
	DoverA := pushFwd(AtoD); --finally D as an A module
	moduleMapAtoASN := pushforward(ASNoverA, sub(1, ASN)); --we have a map A->ASN as A modules
	moduleMapAtoB := pushforward(BoverA, sub(1, B)); --and a map A->B as A modules
	listGensASNinD := apply(first entries pushFwdGens(ASNoverA), tt -> pushforward(DoverA, ASNtoD(tt))); --we take the generators of
	...
	listGensBinD :=  apply(first entries (pushFwdGens(BoverA)), tt -> pushforward(DoverA, BtoD(tt)));
	...
	newVarsTargets := first entries ((pushFwdGens(BoverA))*(g(gens targetSubmodule))); --so we grab these module generators
...
TEST /// --#7 check that the non-semi-normal locus is the right set
	...
	RtoRSN = snList#1;
	pfwd = pushFwd(RtoRSN);
	myMap = pushforward(pfwd, sub(1, target RtoRSN));
...
pfwd = pushFwd(snmap);
assert( isFreeModule (pfwd) and (1 == rank(pfwd)))
///
