...
Node
  ...
  Description
    Text
      ...
      When $S$ is a $\ZZ^r$-graded ring and $M$ a graded $S$-module, then for a (finite)
      subset of degrees $L\subset\ZZ^r$ the method {\tt truncate(L, M)} computes
      $$M_{\ge L} = \bigoplus_{m\in L+\NN^r} M_m,$$ where the sum is taken over all
      degrees $m \in \ZZ^r$ which are component-wise greater than some degree $d\in L$.
      This makes the truncation a submodule of $M$ in the case when all degrees are non-negative.
      In the case when some degree components are negative, this is likely not a submodule.
    Example
      R = ZZ/101[a..d, Degrees => {1,2,3,4}];
      truncate(4, ideal"a3,b3")
    Text
      More generally, let $S$ be the total coordinate ring of a simplicial toric variety
      $X$ with Picard group $\operatorname{Pic} X$, then for any subset $D\subset\operatorname{Pic} X$,
      the truncation $M|_D$ may be defined as the submodule of $M$ generated by all the
      homogeneous elements in $M$ of degree $d\in D$ (c.f. Definition 5.1 in [MS04]).
      Given a finite subset of degrees $L$, we take $D$ to be the semigroup $L + \NN\mathcal C$,
      where $\mathcal C$ is a generating set for the semigroup of Nef line bundles on $X$.
      This definition of truncation preserves exact sequences of graded modules.
    Example
      needsPackage "NormalToricVarieties";
      dP6 = smoothFanoToricVariety(2, 4);
      S = ring dP6;
      truncate({0,0,0,0}, S^{-{0,0,1,0}})
    Text
