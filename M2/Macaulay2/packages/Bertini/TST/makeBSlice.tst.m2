...
xySlice=makeB'Slice(sliceType,variableGroups,BNumberCoefficients=>{{2,3},{5,7,11}})
--Our slice consists of two sections. 
--The ith section is in the variables variableGroups_(sliceType_i)
assert(xySlice#BHyperplaneString_0==="(2)*(x0)+(3)*(x1)")
assert(xySlice#BHyperplaneString_1==="(5)*(y0)+(7)*(y1)+(11)*(y2)")
...
makeB'InputFile(storeBM2Files,
    ...
    BPolynomials=>{f1,f2}|xxSlice#ListBHyperplanes)
runBertini(storeBM2Files)
xxDegree=#importSolutionsFile(storeBM2Files)
assert(xxDegree==2)

xySlice=bSlice({1,1},variableGroups,ContainsMultiProjectivePoint=>{{1,2,3},{4,5,6}},BHomogenization=>{hx,hy},BNumberCoefficients=>{{20,21,22},{30,31,32}})
