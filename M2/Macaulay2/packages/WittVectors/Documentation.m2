...
doc ///
	...
	explicit
    Headline
        Convert Witt objects to explicit presentations.
    Usage
	E = explicit x
    Inputs
	x:Thing
    ...
    Description
	Text
	    In this package, @TO explicit@ converts Witt rings, Witt ideals, and
	    Witt ring maps to ordinary rings, ideals, and maps used for computation.
	Example
	    R = (ZZ/2)[x];
	    W = witt(2, R);
	    explicit W
...
doc ///
	...
	overring
    Headline
        The cached overring of a Witt ring.
    Usage
	WR.overring
    Inputs
	WR:{WittPolynomialRing, WittQuotientRing}
    Outputs
	:Ring
    Description
	Text
	    The overring of a Witt ring is the ambient ring used for explicit
	    computations with Witt vectors.  This value is also returned by
	    @TO wittOverring@.
	Example
	    R = (ZZ/2)[x,y];
	    WR = witt(3, R);
	    WR.overring === wittOverring WR
    SeeAlso
	wittOverring
///
