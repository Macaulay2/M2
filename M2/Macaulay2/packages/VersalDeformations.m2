...
cotangentCohomology1Mod:=(F)->(
     ...
     trim Hom(ideal F,A)/(image ((transpose substitute(jacobian F,A)))))
...
cotangentCohomology2Mod:=F->(
     ...
     R:=gens ker F;
     kos:=koszul(2,F);
     (trim Hom((image R/image kos),A)/(image substitute(transpose R,A))))
...
normalMatrix Matrix:=opts->F->lift(ambient basis(trim Hom(ideal F, ring F/ideal F),opts), ring F)
normalMatrix (ZZ,Matrix):=opts->(deg,F)->lift(ambient basis(deg,trim Hom(ideal F, ring F/ideal F),opts), ring F)
normalMatrix (List,Matrix):=opts->(deg,F)->lift(ambient basis(deg,trim Hom(ideal F, ring F/ideal F),opts), ring F)
normalMatrix (ZZ,ZZ,Matrix):=opts->(lo,hi,F)->lift(ambient basis(lo,hi,trim Hom(ideal F, ring F/ideal F),opts), ring F)
normalMatrix (InfiniteNumber,ZZ,Matrix):=opts->(lo,hi,F)->lift(ambient basis(lo,hi,trim Hom(ideal F, ring F/ideal F),opts), ring F)
normalMatrix (ZZ,InfiniteNumber,Matrix):=opts->(lo,hi,F)->lift(ambient basis(lo,hi.trim Hom(ideal F, ring F/ideal F),opts), ring F)

normalMatrix Ideal:=opts->I->lift(ambient basis(trim Hom(I, ring I/I),opts), ring I)
normalMatrix (ZZ,Ideal):=opts->(deg,I)->lift(ambient basis(deg,trim Hom(I, ring I/I),opts), ring I)
normalMatrix (List,Ideal):=opts->(deg,I)->lift(ambient basis(deg,trim Hom(I, ring I/I),opts), ring I)
normalMatrix (ZZ,ZZ,Ideal):=opts->(lo,hi,I)->lift(ambient basis(lo,hi,trim Hom(I, ring I/I),opts), ring I)
normalMatrix (InfiniteNumber,ZZ,Ideal):=opts->(lo,hi,I)->lift(ambient basis(lo,hi,trim Hom(I, ring I/I),opts), ring I)
normalMatrix (ZZ,InfiniteNumber,Ideal):=opts->(lo,hi,I)->lift(ambient basis(lo,hi.trim Hom(I, ring I/I),opts), ring I)
...
document {
     ...
     PARA {"The matrix ",TT "F"," must have a single row.  Inputing an ideal instead has the same effect as inputing ",TT "gens F",".  The output ",TT "N"," is a matrix
	  over the same ring as ",TT "F"," whose columns form a basis for 
	  (a graded piece of) the normal module ",TT "trim Hom(I,S/I)",",
