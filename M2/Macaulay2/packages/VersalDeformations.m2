...
lowestOrder:=(G,F,C,n)->(
    ...
    T:=newRing(ring G_0,Degrees=>splice {(numgens ring G_0):1},MonomialOrder=>Weights=>splice {(numgens ring G_0):-1},Global=>false);
    I:=ideal sub(sum G,T);
    GB:=flatten entries (gens gb(I,DegreeLimit=>n));
    LO:=apply(GB,f->leastTerm(f,n));
    keep:=positions(LO,i->i=!=null);
    LO=LO_keep;
    GB=GB_keep;
    B:=(sub((matrix {GB}),ring G_0))//(transpose sum G);
...
liftDeformation(List,List,List,List):= opts->(F,R,G,C)->(
     ...
     else lowG=transpose map(source F_0,target C_0,0); --unobstructed case
     T:=ring F_0;
