...
void PolyRing::make_trivial_ZZ_poly_ring()
{
  ...
  trivial_poly_ring->gb_ring_ = GBRing::create_PolynomialRing(flatR->getCoefficientRing(),
                                                              flatR->getMonoid());
}
...
{
  int sign = 0;
  if ((is_skew_ || !use_coeff) && !M_->divides(m, n)) return K_->from_long(0);
