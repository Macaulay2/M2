...
public:
  ...
  size_t numRows() const { return fq_nmod_mat_nrows(mArray, ring().flintContext()); }
  size_t numColumns() const { return fq_nmod_mat_ncols(mArray, ring().flintContext()); }

  const ElementType* array() const { return mArray->entries; }
  ElementType*&  array() { return mArray->entries; }

  ElementType& entry(size_t row, size_t column) { return * fq_nmod_mat_entry(mArray, row, column); }
  const ElementType& entry(size_t row, size_t column) const { return * fq_nmod_mat_entry(mArray, row, column); }
