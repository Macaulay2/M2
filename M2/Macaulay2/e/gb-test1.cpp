                              ...
                              int deg)
{
  ...
  g->gap = deg - f_leadweight; // used to be: deg - weightInfo->gbvector_term_weight(f);
  g->reduced_deg = f_wt;
...
void gbB::gbelem_text_out(buffer &o, int i, int nterms) const
{
  ...
  o << " ["
    ...
    << " size " << gb[i]->size
    << " deg "  << gb[i]->deg
    << " rgap " << gb[i]->reduced_gap
    << " rdeg " << gb[i]->reduced_deg;
  if (ismingen)
    o << " mingen";
  o << "]";
}
...
gbB::spair *gbB::spair_make(int i, int j)
{
  ...
  result->lcm = exponents_make();
    exponents_lcm(nvars, g1->deg, exp1, exp2, result->lcm, gb_weights, result->deg);
    if (g2->gap > g1->gap)
...
struct spair_sorter : public std::binary_function<gbB::spair *,gbB::spair *,bool> {
  int nvars;
...
public:
  ...
  SPolySorterB(GBRing *R0, const FreeModule *F0)
    : F(F0), R(R0), ncmps(0) {}
