...
/**
 * @file monideal.hpp
 * @brief `MonomialIdeal` --- exponent-vector-only representation of an ideal generated by monomials.
 *
 * Declares `MonomialIdeal`, the specialised type the engine
 * uses for ideals whose generators are pure monomials.
 * Generators are not held as a flat list but as a tree of
 * `Nmi_node`s indexed by `(var, exp)`: each internal node
 * branches on a variable and exponent, each leaf carries a
 * `Bag*` whose `monom()` returns the generator as a
 * `gc_vector<int>` in varpower form. The trie shape supports
 * fast divisibility / containment lookups by walking the tree
 * variable-by-variable rather than scanning a list of monomials.
 * The combinatorial structure lets every operation skip
 * coefficient arithmetic and monomial-ordering work entirely:
 * intersection, sum, quotient, saturation, radical, Borel
 * closure, and monomial primary decomposition all reduce to
 * tree walks.
 *
 * The class is `EngineObject`-backed and intentionally
 * immutable once interned (its hash is computed lazily). The
 * `#if 0` block at the top of the header sketches a
 * `SparseMonomial` view interface that has not been implemented
 * --- ignore it when reading the active code. The same monomial
 * shape is what `hilb.hpp`'s Hilbert-series recursion consumes,
 * and the initial-ideal output of a Groebner basis lives here.
 *
