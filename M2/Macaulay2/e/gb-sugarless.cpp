                           ...
                           M2_arrayint gb_weights)
{
  ...
  gb->next = nullptr;             // (both minimal, and large GB's)
  gb->next_min = nullptr;
...
{
  s_pair *r;
  for (r = p->pair_list; r != nullptr; r = r->next_same)
    if (r->second == q)
      {
        if (r->compare_num >= 0)
          {
  ...
  for (r = q->pair_list; r != nullptr; r = r->next_same)
    if (r->second == p)
      {
        if (r->compare_num >= 0)
          {
...
int GBinhom_comp::search(const int *exp, int comp, gb_elem *&result)
{
  int nvars = M->n_vars();
  int *exp2;
  for (gb_elem *p = gbLarge->next; p != nullptr; p = p->next)
    {
...
void GBinhom_comp::gb_insert(gbvector *f, gbvector *fsyz, int minlevel)
{
  ...
  for (;;)
    {
      if (q->next == nullptr || compare(p, q->next) == LT)
        {
          p->next = q->next;
          q->next = p;
          n_gb++;
...
void GBinhom_comp::minimalize_gb()
{
  ...
  for (gb_elem *q = gb->next_min; q != nullptr; q = q->next_min)
    {
      POLY g;
      g.f = q->f;
      g.fsyz = q->fsyz;
...
const Matrix /* or null */ *GBinhom_comp::get_mingens()
{
  ...
  for (gb_elem *q = gb->next; q != nullptr; q = q->next)
...
void GBinhom_comp::debug_pairs_out(buffer &o, gb_elem *p) const
{
  s_pair *q;
  int n = 0;
  for (q = p->pair_list; q != nullptr; q = q->next_same)
    {
      debug_out(o, q);
      n++;
      if (n % 10 == 0) o << newline;
