...
static void write_row(std::vector<int>& grading, int nvars, int which, int value)
{
  for (int i=0; i<nvars; i++)
    if (i == which) grading.push_back(value);
    else grading.push_back(0);
}
static void write_weights(std::vector<int>& grading, int nvars, int firstvar, int* wts, int nwts)
// place nvars ints into grading:  0 ... 0 wts[0] wts[1] ... wts[nwts-1] 0 .... 0
// where wts[0] is in the 'firstvar' location.  If wts is NULL, treat it as the vector with nwts '1's.
{
  for (int i=0; i<firstvar; i++)
    grading.push_back(0);
  if (wts == 0)
    ...
    for (int i=0; i<nwts; i++)
      grading.push_back(wts[i]);
  for (int i=firstvar+nwts; i<nvars; i++)
    grading.push_back(0);
}
