...
INSTANTIATE_TYPED_TEST_SUITE_P(ARingQQGMP,
                               ARingQQ,
...
TEST_F(ARingQQGMP, Conversions)
{
  ...
  // TODO: the result does not always round-trip, because the
  // convergence test in set(double) rounds twice.  Counted rather than
  // asserted per value, so this reports once.
  int badRoundTrips = 0;
  double firstBad = 0.0;
  for (int i = 1; i < 200; i++)
    {
      double d = std::sqrt(static_cast<double>(i));
      ASSERT_TRUE(R.set(a, d));
      if (mpq_get_d(&a.value()) != d)
        {
          if (badRoundTrips == 0) firstBad = d;
          badRoundTrips++;
        }
    }
  EXPECT_EQ(badRoundTrips, 0)
      << "set(double) does not round-trip; first failure at sqrt: "
      << std::setprecision(17) << firstBad;
