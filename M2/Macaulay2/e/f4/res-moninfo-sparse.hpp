...
class ResMonoidSparse
{
  int mNumVars;
  int nslots;
  std::unique_ptr<res_monomial_word[]> hashfcn; // array 0..mNumVars-1 of hash values for each variable
  res_monomial_word mask;
  std::vector<int> mVarDegrees; // array 0..mNumVars-1 of primary (heft) degrees for each variable.
...
public:
  ...
  res_monomial_word hash_value(res_const_packed_monomial m) const { ncalls_hash_value++; return m[1]; }
  ...
  void set_component(component_index component, res_packed_monomial m) const { m[2] = component; }
