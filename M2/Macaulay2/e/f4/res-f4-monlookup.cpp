...
ResF4MonomialLookupTableT<Key>::new_mi_node(varpower_word v, varpower_word e, mi_node *d)
{
...
template <typename Key>
typename ResF4MonomialLookupTableT<Key>::mi_node *
ResF4MonomialLookupTableT<Key>::new_mi_node(varpower_word v, varpower_word e, Key k)
{
  mi_node *p = reinterpret_cast<mi_node *>(mi_stash->new_elem());
  p->var = v;
  p->exp = e;
...
template <typename Key>
ResF4MonomialLookupTableT<Key>::ResF4MonomialLookupTableT(int nvars, stash *mi_stash0)
{
  count = 0;
  mi_stash = mi_stash0;
  if (mi_stash == 0)
...
template <typename Key>
void ResF4MonomialLookupTableT<Key>::insert1(mi_node *&top, const_varpower_monomial b, Key k)
{
  count += 2;
      ...
      if (q->exp != insert_exp)
        {
          ...
          if (i.valid())
            {
              insert_node
                = new_mi_node(insert_var, insert_exp, reinterpret_cast<mi_node *>(NULL));
...
template <typename Key>
void ResF4MonomialLookupTableT<Key>::update_exponent_vector(int topvar, const_varpower_monomial m)
{
  int nvars = topvar + 1;
  if (*m > 0 && m[1] >= nvars)
...
template <typename Key>
void ResF4MonomialLookupTableT<Key>::reset_exponent_vector(const_varpower_monomial m)
{
  int nparts = static_cast<int>(*m++);
  for (int i=nparts; i>0; i--, m+=2)
    {
...
template <typename Key>
void ResF4MonomialLookupTableT<Key>::find_all_divisors_vp(long comp,
                                                    const_varpower_monomial m,
                                                    VECTOR(Key) &result_k) const
{
...
template <typename Key>
bool ResF4MonomialLookupTableT<Key>::find_one_divisor_packed(const ResMonoid *M,
...
template <typename Key>
void ResF4MonomialLookupTableT<Key>::find_all_divisors_packed(const ResMonoid *M,
                                                    const_packed_monomial m,
                                                    VECTOR(Key) &result_k) const
{
  long comp = M->get_component(m);
...
template <typename Key>
void ResF4MonomialLookupTableT<Key>::insert_minimal_vp(long comp,
                                                 const_varpower_monomial m,
                                                 Key k)
{
  if (comp >= mis.size())
...
{
  if (find_one_divisor_vp(comp,m, k))
    return false;
...
template <typename Key>
typename ResF4MonomialLookupTableT<Key>::mi_node *ResF4MonomialLookupTableT<Key>::next(mi_node *p) const
{
  while (p != NULL)
    {
      p = p->left;
...
template <typename Key>
typename ResF4MonomialLookupTableT<Key>::mi_node *ResF4MonomialLookupTableT<Key>::prev(mi_node *p) const
{
  while (p != NULL)
    {
      p = p->right;
...
template <typename Key>
void ResF4MonomialLookupTableT<Key>::do_node(mi_node *p, int indent, int disp) const
{
  buffer o;
  int i;
  assert(p->left != NULL);
...
template <typename Key>
void ResF4MonomialLookupTableT<Key>::do_tree(mi_node *p, int depth, int indent, int disp) const
{
...
template <typename Key>
int ResF4MonomialLookupTableT<Key>::debug_check(mi_node * const p, const mi_node * const up) const
                              ...
                              stash *mi_stash)
{
  ...
  ResF4MonomialLookupTableT<int> M(10, mi_stash); // The 10 is simply a suggested start value
