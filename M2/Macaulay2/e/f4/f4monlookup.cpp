...
template <typename Key>
void F4MonomialLookupTableT<Key>::update_exponent_vector(int topvar, const_varpower_monomial m)
{
  int nvars = topvar + 1;
  if (*m > 0 && m[1] >= nvars)
    nvars = m[1] + 1;
...
template <typename Key>
bool F4MonomialLookupTableT<Key>::find_one_divisor_packed(const MonomialInfo *M,
...
template <typename Key>
typename F4MonomialLookupTableT<Key>::mi_node *F4MonomialLookupTableT<Key>::next(mi_node *p) const
{
  while (p != NULL) 
    {
      p = p->left;
...
template <typename Key>
typename F4MonomialLookupTableT<Key>::mi_node *F4MonomialLookupTableT<Key>::prev(mi_node *p) const
{
  while (p != NULL) 
    {
      p = p->right;
...
template <typename Key>
void F4MonomialLookupTableT<Key>::do_tree(mi_node *p, int depth, int indent, int disp) const
{
...
template <typename Key>
int F4MonomialLookupTableT<Key>::debug_check(mi_node *p, mi_node *up) const
