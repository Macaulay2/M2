...
// flags:
// 1: minimize lead terms.
//   if not set, they are ASSUMED to be already minimial.
//   Otherwise they are computed.
// 2: sort terms, in ascending degree order
//     this could mean a number of things, depending on the ring.
//     otherwise do not sort.
// 4: auto-reduce the elements.
//   If not set, do NOT auto-reduce the generators
//   Otherwise, DO auto-reduce.
// 8: Check: minimality of lead terms, sort order.
//
// Step 1: Sort monomials so that minimalization can easily be done.
// Step 2: Minimalize monomials, creating minimal set.
//  This leaves monomials sorted.  Although optionally use the original ordeer?
// Step 3: Auto-reduce.
//   Either with or without sugar
//
// If we assume the terms are minimal, we can still sort them, or not.
// We can auto-reduce or not.  In fact there are two notions here.
//   Terms can be monic or content free, or other.

#if 0
// elements --> VECTOR(POLY)
// 
  void set_elements(VECTOR(POLY)& polys);
  bool minimize(); //
  void sort(); // sorts the elements into a specific order
    ...
    int flags) /* 1: already minimal. 2: already sorted. 4: do not auto-reduce, 8:check whether minimal, sorted */
{
  ...
  try
    {
      return GBDeclared::create(m, gb, change, syz, flags);
  } catch (exc::engine_error e)
    {
      ERROR(e.what());
      return NULL;
