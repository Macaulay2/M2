...
\ingroup rings
*/
class ARingCCi : public RingInterface
{
 ...
 public:
  ...
  unsigned int computeHashValue(const elem &a) const
  {
    ...
    double d = 12347. * computeHashValue(a.re) + 865800. * computeHashValue(a.im);
  ...
  bool is_zero(const ElementType &f) const { return is_zero(&f.re) and is_zero(&f.im); }
  bool is_equal(const ElementType &f, const ElementType &g) const
  {
    return is_equal(&f.re,&g.re) and is_equal(&f.im,&g.im);
  }

  int compare_elems(const ElementType &f, const ElementType &g) const
  {
    int cmp_re = compare_elems(&f.re, &g.re);
    if (cmp_re != 0) return cmp_re;
    int cmp_im = compare_elems(&f.im, &g.im);
    if (cmp_im != 0) return cmp_im;
    return 0;
  }

    bool is_empty(const ElementType &f) const { return is_empty(&f.re) and is_empty(&f.im); }
  // edited to here
