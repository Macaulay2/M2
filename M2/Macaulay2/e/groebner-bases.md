...
| File pair | Algorithm |
|---|---|
| `gb-default.{cpp,hpp}` | Buchberger-style "default" algorithm — workhorse for general rings |
| `gb-homog2.{cpp,hpp}` | Homogeneous specialisation |
| `gb-sugarless.{cpp,hpp}` | A "sugarless" variant of Buchberger |
| `gb-toric.{cpp,hpp}` | Specialised algorithm for toric ideals |
| `gb-walk.{cpp,hpp}` | Gröbner walk — convert a GB w.r.t. one order to another |
...
|---|---|
| `reducedgb.{cpp,hpp}` | Abstract base |
...
| File pair | Purpose |
|---|---|
| `gbring.{cpp,hpp}` | A polynomial-ring view tuned for GB arithmetic (fast head/tail decomposition) |
| `gbweight.{cpp,hpp}` | Weight-vector tracking during GB (used for early-exit by degree) |
| `spair.{cpp,hpp}` | S-pair data structure: the unit of work in Buchberger-style algorithms |
| `mathicgb-interface.{cpp,hpp}` | Bridge to the [`mathicgb`](../../submodules/README.md) submodule library |
