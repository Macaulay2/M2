...
struct MatrixWindow
{
  ...
  {
    return (end_row-begin_row == b.end_row - b.begin_row) and (end_column - begin_column == b.end_column - b.begin_column);
  }
};
...
struct SubMatrix
{
  ...
  SubMatrix(MatType& m)
    : matrix(m), begin_row(0), end_row(m.numRows()), begin_column(0), end_column(m.numColumns())
  {
  }

  SubMatrix(MatType& m, long x, long y, long nrows, long ncols)
    : matrix(m), begin_row(x), end_row(x+nrows), begin_column(y), end_column(y+ncols)
  {
  }
