...
char *getmem(size_t n)
{
  ...
  p = M2_debug_malloc(n);
#else
  p = reinterpret_cast<char*>(GC_MALLOC(n));		/* GC_MALLOC clears its memory; we preserve that */
#endif
...
char *getmem_clear(size_t n)
{
  ...
  p = M2_debug_malloc(n);
#else
  p = reinterpret_cast<char*>(GC_MALLOC(n));
#endif
  if (p == NULL) outofmem2(n);
#ifdef MEMDEBUG
  memset(p,0,n);
...
#else  
  p = reinterpret_cast<char*>(GC_MALLOC_ATOMIC(n));
#endif
  if (p == NULL) outofmem2(n);
#ifndef NDEBUG
...
// char *getmoremem (char *s, size_t old, size_t newsize) {
//      void *p;
//      enter_getmem();
//      p = reinterpret_cast<char*>(GC_REALLOC(s,newsize));
//      if (p == NULL) outofmem2(newsize);
// #    ifndef NDEBUG
//      trapchk(p);
// #    endif
//      exit_getmem();
...
// char *getmoremem1 (char *s, size_t newsize) {
//      void *p;
//      enter_getmem();
//      p = reinterpret_cast<char*>(GC_REALLOC(s,newsize));
//      if (p == NULL) outofmem2(newsize);
...
// char *getmoremem_atomic (char *s, size_t old, size_t newsize) {
//      void *p;
//      enter_getmem();
// #ifdef MEMDEBUG
//      p = M2_debug_malloc_atomic(newsize);
// #else  
//      p = reinterpret_cast<char*>(GC_MALLOC_ATOMIC(newsize));
// #endif
//      size_t min = old<newsize ? old : newsize;
//      if (p == NULL) outofmem2(newsize);
//      memcpy(p, s, min);
//      /* GC_FREE(s); */
// #    ifndef NDEBUG
//      {
//        int excess = newsize - min;
...
void* M2_debug_realloc(void *old, size_t size) {
     void *newsize = M2_debug_malloc(size);
     size_t oldsize = M2_debug_size(old);
     if (newsize == NULL) outofmem2(size);
     memcpy(newsize,old,size < oldsize ? size : oldsize);
     return newsize;
     }
