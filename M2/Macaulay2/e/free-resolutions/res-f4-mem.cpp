...
const unsigned int resdoublesize[DOUBLESIZE] = { 12, 24, 48, 96, 192, 384, 768,
...
void ResF4Vec::show()
{
  ...
  fprintf(stderr, "index\t\tsize\t\tnalloc\t\tndealloc\t\thighwater\tcurrent\n");
  for (int i=0; i<DOUBLESIZE; i++)
    if (nallocs[i] > 0)
      {
        fprintf(stderr, "%4u\t%12u\t%12u\t%12u\t\t%12u\t%12u\n",
                i,
                resdoublesize[i],
                nallocs[i],
                ndeallocs[i],
