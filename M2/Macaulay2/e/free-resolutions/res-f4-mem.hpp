...
extern const unsigned int resdoublesize[DOUBLESIZE];
...
public:
  ...
  ResF4Vec(const char *name0) : name(name0) { reset(); }
  ...
  f4vec allocate(int alloc_sz) {
    ...
    int nextlen = resdoublesize[s];
    nallocs[s]++;
    current[s]++;
    if (highwater[s] < current[s])
      highwater[s] = current[s];
    int *result = new int[nextlen];
  ...
  void resize(f4vec &a, int newsize) {
    ...
    int s = alloc_stash(a);
    if (resdoublesize[s] < newsize)
      reallocate(a, newsize);
    else
      a[-1] += (newsize-size(a));
  }

  void grow(f4vec &a, int ntoadd) {
    resize(a, size(a)+ntoadd);
  }
...
public:
  ...
  monomial_word * allocate_monomial_array(int len) {
    ...
    monom_total += len;
    monom_current += len;
    if (monom_highwater < monom_current)
      monom_highwater = monom_current;
    monomial_word *result = new monomial_word[len];
    return result;
  }
