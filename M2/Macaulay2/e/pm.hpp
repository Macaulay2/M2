...
struct pm
{
 ...
 public:
  ...
  pm(int numexps0, int numbins0, pmonomial<BIN> identity0)
      : numexps(numexps0), numbins(numbins0), identity(identity0)
  {
  }
  ...
  int compare(pmonomial<BIN> x, pmonomial<BIN> y)
  {
    ...
    while (1)
      {
        if
          expect_false(x[j] > y[j]) return 1;
        else if
          expect_false(x[j] < y[j]) return -1;
        j++;
        n--;
        if
          expect_false(n == 0) return 0;
      }
  }
  virtual int compare_partial(pmonomial<BIN> x, pmonomial<BIN> y, int numparts);
  virtual int Schreyer_compare(vmonomial<BIN> x,
                               vmonomial<BIN> y,
                               pmonomial<BIN> *sch,
                               int *tiebreaker);
  virtual void mult(pmonomial<BIN> dest,
                    const pmonomial<BIN>,
                    const pmonomial<BIN>);
  virtual void mult_unsafe(pmonomial<BIN> dest,
                           const pmonomial<BIN> x,
                           const pmonomial<BIN> y)
  {
    int j = numbins;
    while (--j >= 0)
      dest.contents[j] = x.contents[j] + y.contents[j] - identity.contents[j];
  }
  virtual void div(pmonomial<BIN> dest,
                   const pmonomial<BIN>,
                   const pmonomial<BIN>);
  virtual void div_unsafe(pmonomial<BIN> dest,
                          const pmonomial<BIN> x,
                          const pmonomial<BIN> y)
  {
    int j = numbins;
  ...
  virtual void lcm(pmonomial<BIN> dest,
                   pmonomial<BIN> fac1,
                   pmonomial<BIN> fac2,
                   const pmonomial<BIN>,
                   const pmonomial<BIN>);
  virtual void gcd(pmonomial<BIN> dest,
                   pmonomial<BIN> fac1,
                   pmonomial<BIN> fac2,
                   const pmonomial<BIN>,
                   const pmonomial<BIN>);
...
struct pm_makers
{
  ...
  pm<EXP, BIN> *addweights(int numwts,
                           int bitsperfield,
                           const EXP *wts,
                           pm<EXP, BIN> *);
