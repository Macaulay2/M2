...
export toExpr(x:RawPathTrackerOrNull):Expr := when x is r:RawPathTracker do Expr(r) is null do engineErrorMessage();
...
export toExpr(x:CCorNull):Expr := when x is i:CC do Expr(i) is null do engineErrorMessage();
