...
doc ///
   ...
   Description
    Text
     ...
     The name Jacobian Dual comes from the case where the ideal I is generated by homogeneous
     forms f_0,...,f_m. Let x_1,...,x_n be the variables of R, T_0,...,T_m be independant
     variables, X={x_1,...,x_n}, T={T_0,...,T_m} be row matrices and phi be a matrix 
     of a presentation of I
     
     R^N->R^{m+1}->I->0,
     
     where the first arrow is phi, the second arrow sends T_i to f_i. Then phi_{i,j} is
     f_j/gcd(f_i,f_j) if i<j, -f_i/gcd(f_i,f_j) if i>j and 0 if i=j. Note that phi_{i,j}, if nonzero, is
     always homogeneous, with degree D_{i,j}>0. Thus by Euler's formula we have
     
     D_{i,j}phi_{i,j}= sum(dphi_{i,j}/dx_k x_k).
     
     The equation T*phi=X*psi is actually an equation between 1*N matrices. We shall have
     
     sum(T_i*phi_{i,j})=sum(x_k*psi_{k,j}),j=1,...,N.
     
     So when D_{i,j} are invertible in R and if we set psi_{k,j}=sum(dphi_{i,j}/dx_k*T_i/D_{i,j}),
     we get the jacobian dual psi={psi_{k,j}}, a n*N matrix.
...
restart
uninstallPackage "ReesAlgebra"
path=append(path,"~/Workshop-2017-Berkeley/ReesAlgebras/")
installPackage "ReesAlgebra"
viewHelp jacobianDual
