...
track'option'list = {
	  ...
	  NumericalAlgebraicGeometry$trackbackwards=>false, 
...
--     then error "Certified (experimental) requires Software=>M2 and o.SLP=>false";
     ...
     then (
	  ...
	  if DBG>4 then << "Re<S,T> = " << reBW'ST << ", bigT = " << bigT << endl; 
     ...
     else (if o#(NumericalAlgebraicGeometry$trackbackwards) == false then 
     	  H = transpose (
	      ...
	      + t^(o#(NumericalAlgebraicGeometry$tDegree))*sub(nT.PolyMap,Rt)
     	  ...
     	  H = transpose (
	      o#(NumericalAlgebraicGeometry$gamma)*(t)^(o#(NumericalAlgebraicGeometry$tDegree))*sub(nS.PolyMap,Rt)
	      + (1-t)^(o#(NumericalAlgebraicGeometry$tDegree))*sub(nT.PolyMap,Rt)
	      ); -- row matrix
