...
alexander = method()
alexander(Ideal) := i -> (
     m2 = ideal apply (flatten entries vars ring i, p->p^2);
...
randomSquareFreeStep(Ideal) := o -> (I) ->(
     ...
     if random RR <o.AlexanderProbability then return alexander I;
     S := ring I;
     --Otherwise, it
     --uses the Metropolis algorithm to produce a random walk on the space
     --of square-free ideals. 
     ...
     --and a list of minimal generators Igens of I. 
     mm := ideal vars S;
     mm2 := ideal apply (flatten entries vars S, p->p^2);
     Igens := flatten entries mingens I;
     nI := #Igens;
     ISocgens := flatten entries mingens(((mm2+I):mm)/(mm2+I));
     nSocI := #ISocgens;
     --A candidate "next" ideal J is formed as follows:
     ...
     if random RR < nSocI/(nI+nSocI) then
	  J := trim(I + ideal ISocgens_(random nSocI))
     else (
	  p := random nI;
	  p' := select(toList(0..nI-1), j->j!=p);
	  sqFreeMults := substitute(ideal(gens(mm*ideal(Igens_p))%mm2), S);
	  J = trim (substitute(ideal(Igens_p'),S)+sqFreeMults)
	  );
     ...
     nJ := #Jgens;
     JSocgens := flatten entries mingens(((mm2+J):mm)/(mm2+J));
     nSocJ := #JSocgens;
     P := (nJ+nSocJ)/(nI+nSocI);
...
randomSquareFreeStep(List) := o -> (L) ->(
     if random RR <o.AlexanderProbability then return alexander L_0;
     I := L_0;
     S := ring I;
     mm :=ideal vars S;
     mm2 := ideal apply(flatten entries vars S, p -> p^2);
...
Inputs
  ...
  L:List
   {I,Igens,ISocgens} where I is a square-free monomial ideal,
   Igens is a List of its minimal generators,
   ISocgens is a List of the minimal generators of the socle mod I.
Outputs
  M:List
   {J,Jgens,JSocgens} where J is a square-free monomial ideal,
...
Description
 Text 
  ...
  With 4 variables and 168 possible monomial ideals, a run of 100
  takes less than 2 seconds on a reasonably fast machine. With
  7 variables a run of 1000 takes about 43 seconds.
 Example
  S=ZZ/2[vars(0..3)]
  J = ideal"ab,ad, bcd"
  randomSquareFreeStep J
 Text
  To see how fast this converges, try things like the following, with larger numbers of variables
  and more iterations. To save space we introduce an abbreviation rsfs
 Example
  rsfs = randomSquareFreeStep
  setRandomSeed 1
  J = ideal 0_S
  T=tally for t from 1 to 100 list (J=rsfs(J,AlexanderProbability => .01); if class J === Ideal then J else J_0)
  #T
  J
///
...
Key
  ...
  (alexander, Ideal)
Headline
  the alexander dual of a square-free monomial ideal
Usage
  J = alexander I
Inputs
  I:Ideal
...
Description
 Text
  If I corresponds, under the Stanley-Reisner correspondence,
  to a simplicial complex C, then
  J corresponds, 
  to the Alexander dual simplicial complex C', whose faces are the 
  complements of the non-faces of C.  
  Thus the minimal primes of J are generated by the list of
  variables corresponding to the monomial generators of I.
  By a theorem of Terai, the regularity of J is the projective
  dimension of R/I, and vice versa. There are many related results.
 Example
  S=ZZ/2[vars(0..3)]
  I = ideal"ab,ad, bcd"
  J=alexander I
  betti res I -- pd R/I = 2, reg I = 3
  betti res J -- reg J = 2, pd R/J = 3
///
...
S=ZZ/2[vars(0..3)]
J = ideal"ab,ad, bcd"
assert (randomSquareFreeStep J=={ideal (a*d, a*c, a*b, b*c*d), {a*d, a*c, a*b, b*c*d}, {a, c*d, b*d, b*c}})
S=ZZ/2[vars(0..3)]
I = ideal"ab,ad, bcd"
assert(alexander I==ideal (b*d, a*d, a*c, a*b))
///


end  

--the following doesn't return a reliable number of generators -- the low degree
ones added later sometimes eclipse the earlier ones.

randomSquareFreeMonomialIdeal(ZZ, Ring) := Ideal => (N,S)->(
     --N is a list of the number of monomials desired. The routine
     --successively picks N random numbers 2 <= d <= numgens S, and random square-free monomials
     --of that degree not yet in the ideal produced, unless all monomials of the next
     --degree chosen are already in the ideal, in which case the routine tries with the next
     --lowest degree, etc.
     -- CAVEAT: for very large numbers of variables in the ring the routine should be rewritten.
     M := for i from 2 to numgens S list flatten entries gens squareFree(i,S);
     n := numgens S;
     I := ideal(0_S);
     for t from 1 to N do (
	  d := 2+random(n-1); -- random number in the range 2..n
	  while d>1 do(
	       << d <<endl;
	  Mtmp := flatten entries compress((matrix{M_(d-2)})%I);
	  u := #Mtmp;
	  	       << u <<endl;
	  if u==0  then (d=d-1) else (
	       newgen :=Mtmp_(random u);
	       << newgen << endl;
	       I= I+ideal newgen;
	       break)
	       )
	  );
     if numgens(J:= trim I) < N then error();
     J)
