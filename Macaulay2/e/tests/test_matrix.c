...
void test_matrix_ops(const Matrix *M, const Matrix *N)
     /* M, N should be square matrices */
{
  ...
  assert(IM2_Matrix_is_equal(P1,P2));
  assert(IM2_Matrix_is_zero(IM2_Matrix_subtract(P1,P2)));

  Mdual = IM2_Matrix_transpose(M);
  Mdual2 = IM2_Matrix_transpose(Mdual);
  assert(IM2_Matrix_is_equal(M, Mdual2));
  assert(IM2_Matrix_is_equal(IM2_Matrix_add(M,Mdual),
			     IM2_Matrix_add(Mdual,M)));
  assert(IM2_Matrix_is_equal(IM2_Matrix_transpose(IM2_Matrix_mult(M,Mdual)),
			     IM2_Matrix_mult(IM2_Matrix_transpose(Mdual),
					     IM2_Matrix_transpose(M))));
  assert(IM2_Matrix_is_equal(IM2_Matrix_negate(IM2_Matrix_subtract(M,Mdual)),
...
void test_matrix_ZZ()
{
  ...
  assert(is_eq(IM2_Matrix_to_string(M),
	       "0 6  12 18 24 \n"
	       "1 7  13 19 25 \n"
	       "2 8  14 20 26 \n"
	       "3 9  15 21 27 \n"
