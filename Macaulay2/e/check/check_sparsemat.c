...
void test_sparsemat_ZZ(Test *pTest)
{
  int i,j;
  M2_arrayint C;
  MutableMatrix *Nr,*Nc;
  ...
  P1 = IM2_Matrix_mult(IM2_Matrix_mult(Nr1,M0),
		       Nc1);
  display_matrix(P1);
  ct_test(pTest,IM2_Matrix_is_zero(IM2_Matrix_subtract(M1,P1)));

}

void test_sparsemats(Test *pTest)
{
  test_sparsemat_ZZ(pTest);
}

Test * sparsemat_test(void)
{
  Test *result = ct_create("sparsemat tests", 0);
  ct_addTestFun(result, test_sparsemats);
  return result;
}
