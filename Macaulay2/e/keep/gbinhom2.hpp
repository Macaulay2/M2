...
class GBinhom2_comp : public gb_comp
{
private:
  // Ring information
  const monoid *M;
  ...
  array<TermIdeal *> largeGB;
  array<TermIdeal *> minimalGB;  // baggage for both are gb_elem's
...
private:
  ...
  int gb_reduce(vec &f, vec &fsyz);
  int gb_geo_reduce(vec &f, vec &fsyz);
...
public:
  // Forcing a GB
  GBinhom2_comp(const Matrix &m, const Matrix &gb, const Matrix &mchange, 
	  const Matrix &syz);

  // An honest GB computation
  GBinhom2_comp(const Matrix &m, int collect_syz, int n_syz, int strategy);
  ~GBinhom2_comp();
