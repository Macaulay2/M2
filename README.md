...
| [`BUILD.md`](BUILD.md) | End-to-end build pipeline (cmake → M2-binary) across the seven phases | Setting up a build; debugging build failures; understanding the two build systems |
| [`STARTUP.md`](STARTUP.md) | End-to-end boot path (`main()` → ready prompt) | Debugging startup crashes; understanding what runs when |
| [`MEMORY.md`](MEMORY.md) | End-to-end memory-management story across the five layers (Boehm GC → allocator hooks → pool/slab → external-lib wrappers → overflow-checked arithmetic) | Debugging OOM or memory corruption; understanding allocator choices |
...
- **Per-layer architecture references** — standalone "how does this layer fit together" docs:
   ...
   [`system/architecture.md`](M2/Macaulay2/system/architecture.md) (thread supervisor)
- **Engine subdir architecture references** (covering every major engine subdir):
...
- Packages: [`packages/`](M2/Macaulay2/packages/README.md) (with foundational deep dives for [`Macaulay2Doc`](M2/Macaulay2/packages/file-Macaulay2Doc.md) · [`Style`](M2/Macaulay2/packages/file-Style.md) · [`EngineTests`](M2/Macaulay2/packages/file-EngineTests.md) · [conventions](M2/Macaulay2/packages/file-package-conventions.md), and **all 17 auto-loaded packages**: [`MinimalPrimes`](M2/Macaulay2/packages/file-MinimalPrimes.md) · [`PrimaryDecomposition`](M2/Macaulay2/packages/file-PrimaryDecomposition.md) · [`Saturation`](M2/Macaulay2/packages/file-Saturation.md) · [`Elimination`](M2/Macaulay2/packages/file-Elimination.md) · [`Complexes`](M2/Macaulay2/packages/file-Complexes.md) · [`SimpleDoc`](M2/Macaulay2/packages/file-SimpleDoc.md) · [`Varieties`](M2/Macaulay2/packages/file-Varieties.md) · [`IntegralClosure`](M2/Macaulay2/packages/file-IntegralClosure.md) · [`ReesAlgebra`](M2/Macaulay2/packages/file-ReesAlgebra.md) · [`LLLBases`](M2/Macaulay2/packages/file-LLLBases.md) · [`InverseSystems`](M2/Macaulay2/packages/file-InverseSystems.md) · [`Isomorphism`](M2/Macaulay2/packages/file-Isomorphism.md) · [`Classic`/`ConwayPolynomials`/`OnlineLookup`/`PackageCitations`/`TangentCone` (batched)](M2/Macaulay2/packages/file-utility-packages.md))
...
- **End-to-end startup walkthrough: [`STARTUP.md`](STARTUP.md)** — the eight phases from kernel calling `main()` through engine init, supervisor init, interpreter init, embedded `startup.m2`, Core load, to ready prompt — with failure modes per phase and approximate timing
- **End-to-end memory model: [`MEMORY.md`](MEMORY.md)** — the five-layer story (Boehm GC → allocator hooks → pool/slab → external-lib wrappers → overflow-checked arithmetic), when to use each allocator, why M2 generally doesn't leak, and a symptom-driven debugging table
- **End-to-end testing reference: [`TESTING.md`](TESTING.md)** — the six test infrastructures (engine gtest, `M2 --check N`, per-package `check`, `tests/normal/`, `tests/slow/`, `tests/ComputationsBook/`), what each tests at which layer, how to run them, and the test-architecture diagram
...
The documentation tree comprises **640 markdown files** across four
layers:
...
| Per-file deep dives | 520 | `file-<basename>.md` alongside each source file (some consolidated per family) |
...
| [packages/file-Saturation.md](M2/Macaulay2/packages/file-Saturation.md) | `Saturation.m2` — auto-loaded `saturate` / `quotient` / `annihilator` (with `addHook` strategy tables) |
| [packages/file-Elimination.md](M2/Macaulay2/packages/file-Elimination.md) | `Elimination.m2` — auto-loaded `eliminate` / `resultant` / `discriminant` / `sylvesterMatrix` |
| [packages/file-Complexes.md](M2/Macaulay2/packages/file-Complexes.md) | `Complexes.m2` — auto-loaded `HomologicalAlgebraPackage`: `Complex` / `ComplexMap` / `freeResolution` / `Ext` / `Tor` / Yoneda / pruning |
| [packages/file-SimpleDoc.md](M2/Macaulay2/packages/file-SimpleDoc.md) | `SimpleDoc.m2` — auto-loaded `doc ///…///` indentation-aware doc DSL used by virtually every package |
| [packages/file-Varieties.md](M2/Macaulay2/packages/file-Varieties.md) | `Varieties.m2` — auto-loaded algebraic-geometry types (`Variety`/`AffineVariety`/`ProjectiveVariety`/`CoherentSheaf`/`SheafMap`) + `Spec`/`Proj`/`sheaf`/`tangentSheaf`/`HH^i` |
...
| [packages/file-LocalRings.md](M2/Macaulay2/packages/file-LocalRings.md) | `LocalRings.m2` — operations over localised rings `R_P`: `localRing`, `liftUp`, `localResolution`, `hilbertSamuelFunction`, lift-and-descend pattern |
| [packages/file-BernsteinSato.md](M2/Macaulay2/packages/file-BernsteinSato.md) | `BernsteinSato.m2` — `globalBFunction f`, multiplier ideals `J(f^c)`, D-module algorithms (~6 600 lines, 15 algorithmic modules); JSAG-ish for D-modules in M2 |
...
| [packages/file-PHCpack.md](M2/Macaulay2/packages/file-PHCpack.md) | `PHCpack.m2` — JSAG-certified interface to the PHCpack polyhedral-homotopy solver: `solveSystem`/`mixedVolume`/`cascade`/`numericalIrreducibleDecomposition`; backs `Software => PHCPACK` in NAG |
